...
class member_vr_orderControl extends mobileMemberControl {
    ...
    public function order_listOp() {
        ...
        foreach ($order_list as $key => $order) {
            //显示取消订单
            $order_list[$key]['if_cancel'] = $model_order->getOrderOperateState('buyer_cancel',$order);
        
            //显示支付
            $order_list[$key]['if_pay'] = $model_order->getOrderOperateState('payment',$order);

            //显示评价
            $order_list[$key]['if_evaluation'] = $model_order->getOrderOperateState('evaluation',$order);
            ...
            if($order['nowtime'] != ''){
                $order_list[$key]['time'] = date('Y-m-d',$order['nowtime']);
            }else if(intval($order['nowtime']) == 0 || intval($order['nowtime']) == ''){
                $order_list[$key]['time'] = $order['apptime'];
            }else{
                $order_list[$key]['time'] = date('Y-m-d',$order['apptime']);
            }
