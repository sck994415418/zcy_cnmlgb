...
class buy_1Logic {
    ...
    public function calcCartList($store_cart_list) {
        ...
        foreach ($store_cart_list as $store_id => $store_cart) {
            ...
            foreach ($store_cart as $key => $cart_info) {
				...
				if($is_jibie == 1){
					if($cart_info['goods_vipprice'] != '0.00'){
						$cart_info['goods_price']  = $cart_info['goods_vipprice'];
					}
				}elseif($is_jibie == 2){
					if($cart_info['goods_gjprice'] != '0.00'){
						$cart_info['goods_price']  = $cart_info['goods_gjprice'];
					}
				}
            ...
            $store_cart_list[$store_id] = $store_cart; //店铺购物车
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            foreach ($goods_list as $goods) {
                $brand_goods[$goods['brand_id']][] = $goods;
            }
            $store_goods[$store_id] = $brand_goods;
        }
        //将店铺的商品按照--品牌--将价格叠加起来:得到商品品牌的总价格
        foreach ($store_goods as $store_id1 => $brand_goods1) {
            $brand_total_price = array();
            foreach ($brand_goods1 as $brand_id1 => $goods_list1) {
                $goods_price = 0;
                foreach ($goods_list1 as $goods1) {
                    $goods_price += $goods1['goods_price'] * $goods1['goods_num'];
                }
                $brand_total_price[$brand_id1] = $goods_price;
            }
            $store_brand_price[$store_id1] = $brand_total_price;
        }
        //print_r($store_total_price);die;
        return array($store_cart_list, $store_goods_total, $store_brand_price);
    }
    ...
    public function reCalcGoodsTotal($store_goods_total, $preferential_array, $preferential_type) {
        ...
        switch ($preferential_type) {
            ...
            case 'voucher':
                if (!C('voucher_allow'))
                    return $store_goods_total; //由后台控制
                foreach ($preferential_array as $store_id => $voucher_info) {
                    foreach($voucher_info as $voucher_price) {
                        $store_goods_total[$store_id] -= $voucher_price['voucher_price'];
                    }
                }
                break;
    ...
    public function getStoreAvailableVoucherList($store_goods_total, $member_id) {
        //将settion中config导入置“全部变量”
        if (!C('voucher_allow'))
            return $store_goods_total;
        $voucher_list = array();
        $model_voucher = Model('voucher'); //代金券
        foreach ($store_goods_total as $store_id => $goods_total) {
            $condition = array();
            $condition['voucher_type'] = 0; //全品类代金券
            $condition['voucher_store_id'] = $store_id;
            $condition['voucher_owner_id'] = $member_id;
            $voucher_list[$store_id] = $model_voucher->getCurrentAvailableVoucher($condition, $goods_total); //条件、商品价格
        }
        return $voucher_list;
    }
     ...
     * @return array
     */
    public function getStoreAvailableVoucherBrandList($store_goods_total, $member_id) {
        //将settion中config导入置“全部变量”
        if (!C('voucher_allow'))
            return $store_goods_total;
        $voucher_list = array();
        //print_r($store_goods_total);
        $model_voucher = Model('voucher'); //代金券
        foreach ($store_goods_total as $store_id => $brand_total) {
            
            foreach ($brand_total as $brand_id=>$goods_total) {
                $condition = array();
                $condition['voucher_type'] = 2; //品牌类代金券
                $condition['voucher_store_id'] = $store_id;
                $condition['voucher_owner_id'] = $member_id;
                $voucher_list = $model_voucher->getCurrentAvailableVoucher($condition, $goods_total); //条件、商品价格 
                //print_r($voucher_list);die;
                foreach ($voucher_list as $voucher_t_id => $voucher) {          //$voucher是以模板id为标准
                    $voucher_brand[$voucher['voucher_type_id']][$voucher_t_id] = $voucher;
                }
            }
            //print_r($voucher_brand);die;
            $voucher_brand_list[$store_id] = $voucher_brand;
        }
        return $voucher_brand_list;
    }
    ...
    /**
     * 验证传过来的代金券是否可用有效:有效,则赋值;如果无效，直接删除;
    ...
    public function reParseVoucherList($input_voucher_list = array(), $store_goods_total = array(), $member_id) {
        ...
        foreach ($input_voucher_list as $store_id => $voucher) {
            ...
            if (is_array($tmp) && isset($tmp[$voucher['voucher_t_id']])) {
                $input_voucher_list[$store_id][0]['voucher_id'] = $tmp[$voucher['voucher_t_id']]['voucher_id'];
                $input_voucher_list[$store_id][0]['voucher_code'] = $tmp[$voucher['voucher_t_id']]['voucher_code'];
                $input_voucher_list[$store_id][0]['voucher_owner_id'] = $tmp[$voucher['voucher_t_id']]['voucher_owner_id'];
            } else {
    ...
    /**
     * 验证传过来的代金券是否可用有效:有效,则赋值;如果无效，直接删除;
     * @param array $input_voucher_list 代金券品品牌列表
     * @param array $store_goods_total (店铺ID=>商品总金额)
     * @return array
     */
    public function reParseVoucherBrandList($input_voucher_list = array(), $store_goods_total = array(), $member_id) {
        if (empty($input_voucher_list) || !is_array($input_voucher_list))
            return array();
        //根据商铺总价格:获取全品代金券
        $store_voucher_list = $this->getStoreAvailableVoucherBrandList($store_goods_total, $member_id); //商家id、会员id
        // print_r($store_voucher_list);die;
        //上传的只有代金券的id，根据id
        foreach ($input_voucher_list as $store_id => $brand_voucher) {
            foreach ($brand_voucher as $brand_id => $voucher) {
                //print_r($voucher);die;
                $tmp = $store_voucher_list[$store_id][$brand_id];
                //print_r($tmp);die;
                if (is_array($tmp) && isset($tmp[$voucher['voucher_t_id']])) {
                    // echo 11;
                    $input_voucher_list[$store_id][$brand_id]['voucher_id'] = $tmp[$voucher['voucher_t_id']]['voucher_id']; //代金券模板id
                    $input_voucher_list[$store_id][$brand_id]['voucher_code'] = $tmp[$voucher['voucher_t_id']]['voucher_code'];
                    $input_voucher_list[$store_id][$brand_id]['voucher_owner_id'] = $tmp[$voucher['voucher_t_id']]['voucher_owner_id'];
                } else {
                    unset($input_voucher_list[$store_id][$brand_id]);
                }
            }
        }
        return $input_voucher_list;
...
//         if (empty($pd_payment_info)) return;
    ...
    private function _getGoodsOnlineInfo($goods_id, $quantity) {
		...
		$new_array['goods_vipprice'] = $goods_info['vipprice'];
		$new_array['goods_gjprice'] = $goods_info['gjprice'];
    ...
    private function _getOnlineCartList($cart_list) {
        ...
        foreach ($cart_list as $key => $cart_info) {
            if (!intval($cart_info['bl_id'])) { //“组合套餐id”，不是组合套餐，则执行
        ...
        foreach ($goods_online_list as $goods) {
            $goods_online_array[$goods['goods_id']] = $goods; //保存为键：商品id；值：商品信息
        }
        foreach ((array) $cart_list as $key => $cart_info) {
            if (intval($cart_info['bl_id']))//如果是套餐，则跳过
                continue;
            $cart_list[$key]['state'] = true;
            $cart_list[$key]['storage_state'] = true;
            if (in_array($cart_info['goods_id'], array_keys($goods_online_array))) {  //购物车中，商品的“数量”是不变的，但是商品的其他属性都可能变化。          
                ...
                //新增开始
                $cart_list[$key]['gc1_id'] = $goods_online_info['gc1_id'];
                $cart_list[$key]['gc2_id'] = $goods_online_info['gc2_id'];
                $cart_list[$key]['gc3_id'] = $goods_online_info['gc3_id'];
                $cart_list[$key]['brand_id'] = $goods_online_info['brand_id'];
				
				$cart_list[$key]['goods_vipprice'] = $goods_online_info['vipprice'];
				$cart_list[$key]['goods_gjprice'] = $goods_online_info['gjprice'];
                //新增结束
                ...
                if ($cart_info['goods_num'] > $goods_online_info['goods_storage']) { //如果购物车数量>商品goods表库存，则表示库存不足
    ...
    public function getGroupbuyCartList(& $cart_list) {
        ...
        $model_goods = Model('goods');
        foreach ($cart_list as $key => $cart_info) {
            if ($cart_info['bl_id'] === '1' || empty($cart_info['groupbuy_info']))
                continue;
    ...
    public function getXianshiCartList(& $cart_list) {
        if (!C('promotion_allow') || empty($cart_list))
            return;
        foreach ($cart_list as $key => $cart_info) {
            if ($cart_info['bl_id'] === '1' || empty($cart_info['xianshi_info']))
                continue;
