...
class buyLogic {
     ...
     * @param unknown $ifcart：是否由购物车购买、或者直接购买
    ...
    public function buyStep1($cart_id, $ifcart, $member_id, $store_id) {
        ...
        //得到购买商品信息
        if ($ifcart) {
            $result = $this->getCartList($cart_id, $member_id); //购物车，购买。
        } else {
            $result = $this->getGoodsList($cart_id, $member_id, $store_id); //直接购买，是从商店获取信息
        }
    ...
    public function getBuyStep1Data($member_id, $data) {
        list($goods_list, $store_cart_list) = $data; //将数组的值赋值给变量
        $goods_list = $data['goods_list']; //商品信息
        $store_cart_list = $data['store_cart_list']; //购物车信息：商品、买家、卖家--具有唯一性的cartid
        //定义返回数组
        $result = array();
        //商品金额计算(分别对每个商品/优惠套装小计、每个店铺小计)
        list($store_cart_list, $store_goods_total) = $this->_logic_buy_1->calcCartList($store_cart_list);
        $result['store_cart_list'] = $store_cart_list; //商店购物车列表
        $result['store_goods_total'] = $store_goods_total; //商品总金额。
        ...
        //返回店铺可用的代金券（按照总金额，获取全品类的代金券）
        $store_voucher_list = $this->_logic_buy_1->getStoreAvailableVoucherList($store_goods_total, $member_id); //（一维数组）商店id=》总金额，会员id
        $result['store_voucher_list'] = $store_voucher_list;
        // header("content-type:text/html;charset=utf-8");
        //返回店铺可用代金券（按照分类金额，获取对应类型的代金券）
        //print_r($store_cart_list);二维数组，将二维数组
        // $store_voucher_list = $this->_logic_buy_1->getStoreAvailableVoucherTypeList($store_goods_total, $member_id);
    ...
    private function _createOrderStep1() {
        ...
        //是否开增值税发票
        $input_if_vat = $this->buyDecrypt($post['vat_hash'], $this->_member_info['member_id']); //解密处理
        ...
        //验证代金券:全品类(一维数组)和品牌类(二维数组):代金券id/商店id/代金券价格
        //1.通过修改前台,select就算不选,也会有[0]:0->全品类;[大于0]->品牌类
        //2.这个的两者不能同时存在,
        $input_voucher_list = array();
        if (!empty($post['voucher']) && is_array($post['voucher'])) {
            foreach ($post['voucher'] as $store_id => $brand_voucher) {
                $voucher_array = explode("|", $brand_voucher[0]);
                $voucher_t_id = $voucher_array[0];
                //如果叠加,则抛出异常
                if (isset($brand_voucher[0]) && !empty($voucher_t_id) && count($brand_voucher) > 1) {
                    throw new Exception('全品代金券和品牌代金券不能叠加使用'); //-----抛出到html页面
                }
                foreach ($brand_voucher as $brand_id => $voucher) {
                    //全品类
                    if ($brand_id == 0) {
                        if (preg_match_all('/^(\d+)\|(\d+)\|([\d.]+)$/', $voucher, $matchs)) { //全文匹配
                            if (floatval($matchs[3][0]) > 0) {
                                $input_voucher_list[$store_id][0]['voucher_t_id'] = $matchs[1][0];
                                $input_voucher_list[$store_id][0]['voucher_price'] = $matchs[3][0];
                            }
                        }
                    } else {
                        //品牌类代金券
                        if (preg_match_all('/^(\d+)\|(\d+)\|([\d.]+)$/', $voucher, $matchs)) { //全文匹配
                            if (floatval($matchs[3][0]) > 0) {
                                $input_voucher_list[$store_id][$brand_id]['voucher_t_id'] = $matchs[1][0];
                                $input_voucher_list[$store_id][$brand_id]['voucher_price'] = $matchs[3][0];
                            }
                        }
                    }
                }
    ...
    private function _createOrderStep3() {
        ...
        //print_r($input_voucher_list);die;
        $input_city_id = $this->_order_data['input_city_id'];

        //商品金额计算(分别对每个商品/优惠套装小计、每个店铺小计、每个店铺中每个分组的总价格)
        list($store_cart_list, $store_goods_total, $store_brand_price) = $this->_logic_buy_1->calcCartList($store_cart_list);
        ...
        //将全品类代金券和品牌类代金券分开：传入两个不同的函数，最后再合并为一个数组
        foreach ($input_voucher_list as $store_id => $brand_voucher) {
            $input_voucher_all[$store_id] = $brand_voucher[0];
            unset($input_voucher_list[$store_id][0]);
        }
        //得到有效的:全品类代金券
        $input_voucher_all_list = $this->_logic_buy_1->reParseVoucherList($input_voucher_all, $store_goods_total, $this->_member_info['member_id']);
        //print_r($input_voucher_all_list);
        //得到有效地:品牌代金券
        $input_voucher_brand_list = $this->_logic_buy_1->reParseVoucherBrandList($input_voucher_list, $store_brand_price, $this->_member_info['member_id']);
//        print_r($input_voucher_brand_list);die;
        //将两种代金券进行合并
        $input_voucher_list = $input_voucher_all_list + $input_voucher_brand_list;
        //print_r($input_voucher_list);die;
        //print_r($store_final_goods_total);
        ...
        //print_r($store_final_goods_total);die;
        ...
        //print_r($store_final_order_total);die;
        //计算店铺分类佣金[改由任务计划]
        // 33hao 
...
/*鏌ユ壘goods_commonid  寮�濮�*/    	
////////////////////////////////////////////////////
	$infoArray=$this->_order_data; 
	$a=$infoArray["store_cart_list"][51][0];
	if(array_key_exists("goods_commonid",$a)){
		for($i=0;$i<count($infoArray["store_cart_list"][51]);$i++){
			$goods_list[$i]['goods_commocid']= $infoArray["store_cart_list"][51][$i]["goods_commonid"];
		}
		//$goods_commonid=count($infoArray["store_cart_list"][51]);	 	
	 }else{
	 	$goods_commonid=$infoArray["goods_list"][0]["goods_commonid"];
	 }                                        /// $infoArray["goods_list"][0]["goods_commonid"]     //string(6) "100643"
	 											///$infoArray["store_cart_list"][51][0]["goods_commonid"]   //string(6) "100643" //璐墿杞︿拱
//var_dump($infoArray["store_cart_list"][51][0]["goods_commonid"]);exit;  
//var_dump($goods_list);exit;
/////////////////////////////////////////////////
/*鏌ユ壘goods_commonid  缁撴潫*/

        extract($this->_order_data);                  ///_order_data     涓嬪崟鏁版嵁    

        $member_id = $this->_member_info['member_id'];       //$_member_info     浼氬憳淇℃伅
        $member_name = $this->_member_info['member_name'];            ///////string(6) "123456"        
        ...
        $pay_sn = $this->_logic_buy_1->makePaySn($member_id); //根据用户id生成支付单号
        $order_pay = array();
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            if (isset($input_voucher_list[$store_id])) {
                $voucher_price = array();
                $voucher_code = array();
                foreach ($input_voucher_list[$store_id] as $brand_id => $voucher_list) {
                    $voucher_price[$brand_id] = json_encode($input_voucher_list[$store_id][$brand_id]['voucher_price']); //金额:一个品牌对应一个代金券
                    $voucher_code[$brand_id] = json_encode($input_voucher_list[$store_id][$brand_id]['voucher_code']); //编码   
                }
                //print_r($voucher_price);die;
                $order_common['voucher_price'] = $voucher_price;
                $order_common['voucher_code'] = $voucher_code;
            }
            ...
            foreach ($goods_list as $goods_info) {  
                ...
                if (!intval($goods_info['bl_id'])) {
                    //如果不是优惠套装
                    $order_goods[$i]['order_id'] = $order_id;//订单id
                    $order_goods[$i]['goods_id'] = $goods_info['goods_id'];//商品id
                    $order_goods[$i]['store_id'] = $store_id;//商店id
...
/////////////////////////////////////////////////////
					$order_goods[$i]['goods_commonid'] = $goods_info['goods_commonid'];
///////////////////////////////////////////////////////					
    ...
    private function _createOrderStep6() {
        ...
        $input_voucher_list = $this->_order_data['input_voucher_list'];//属性
        $store_cart_list = $this->_order_data['store_cart_list'];
        $input_buy_items = $this->_order_data['input_buy_items'];
    ...
    public function buyEncrypt($string, $member_id) {
        ...
        if (is_array($string)) {
            $string = serialize($string); //如何是“数组”，则转化为
        } else {
            $string = strval($string); //如何不是“数组”，则强制转化为“字符串”
        }
