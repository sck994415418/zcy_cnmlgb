...
class rent_classModel extends Model
{
    /**
     * 缓存数据
     */
    ...
    /**
     * 缓存数据 原H('rent_class')形式
     */
    protected $gcForCacheModel;

    public function __construct() {
        parent::__construct('rent_class');
    }
    ...
    public function getGoodsClassList($condition, $field = '*') {
        $result = $this->table('rent_class')->field($field)->where($condition)->order('gc_parent_id asc,gc_sort asc,gc_id asc')->limit(false)->select();
        return $result;
    }
    ...
    public function delGoodsClassByGcIdString($gcids) {
        ...
        Model('goods_rent')->editProducesLockUp(array('goods_stateremark' => '商品分类被删除，需要重新选择分类'), array('gc_id' => array('in', $gcid_array)));
        return true;
    }
    ...
    public function getKeyWords($gc_id = null){
        ...
        if ($gc_id > 0){
            ...
            $goods_class = Model('rent_class')->getGoodsClassIndexedListAll();
