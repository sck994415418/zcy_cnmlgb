...
class goods_classModel extends Model{
    ...
    public function getCache() {
        ...
        $data = rkcache('gc_class');//读取缓存
        if (!$data) { //如果没有cache
            $data = array();
            foreach ((array) $this->getGoodsClassList(array('gc_show'=>1)) as $v) {
                $id = $v['gc_id'];
                $pid = $v['gc_parent_id'];
                $data['data'][$id] = $v;
                $data['parent'][$id] = $pid;
    ...
    public function getGoodsClassListAll() {
        $data = $this->getCache();
        return array_values((array) $data['data']);//取得数组的值.
    }
    ...
    public function get_all_category($update_all = 0) {
        ...
        // 不存在时更新或者强制更新时执行
        if ($update_all == 1 || !($gc_list = rkcache('all_categories'))) { //删除缓存
            ...
            wkcache('all_categories', $gc_list);//写入缓存
        }
