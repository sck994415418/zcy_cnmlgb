...
class storeModel extends Model {
    ...
    public function getHotSalesList($store_id, $limit = 5) {
        ...
        if(empty($hot_sales_list)) {
           ...
           //modified by zniu 2018.11.13
            $fieldstr = "goods_id,goods_commonid,goods_name,goods_jingle,store_id,store_name,goods_price,goods_promotion_price,goods_marketprice,goods_storage,goods_image,goods_freight,goods_salenum,color_id,evaluation_good_star,evaluation_count,goods_promotion_type,vipprice,gjprice";
            $hot_sales_list = $model_goods->getGoodsListByColorDistinct(array('store_id' => $store_id), $fieldstr, 'goods_salenum desc', $limit);
            $cache = array();
    ...
    public function getHotCollectList($store_id, $limit = 5) {
        ...
        if(empty($hot_collect_list)) {
            ...
            // modified by zniu 2018.01.13
            $fieldstr = "goods_id,goods_commonid,goods_name,goods_jingle,store_id,store_name,goods_price,goods_promotion_price,goods_marketprice,goods_storage,goods_image,goods_freight,goods_salenum,goods_collect,color_id,evaluation_good_star,evaluation_count,goods_promotion_type,vipprice,gjprice";
            $hot_collect_list = $model_goods->getGoodsListByColorDistinct(array('store_id' => $store_id), $fieldstr, 'goods_collect desc', $limit);
...
/**
    ...
    private function getGoodsListBySales($store_array) {
    	...
    	$field = 'goods_id,store_id,goods_name,goods_image,goods_price,sum(goods_salenum) as goods_salenum,vipprice,gjprice';
    	...
    	foreach ($store_array as $value) {
    		...
    		$store_array[$value['store_id']]['search_list_goods'] = $model->table('goods')->field($field)->where(array('store_id'=>$value['store_id'],'goods_state'=>1))->group('goods_commonid')->order('goods_salenum desc')->limit(8)->select();
                //modified by zniu 2018.01.13
               // $store_array[$value['store_id']]['search_list_goods'] = $model_goods->getGoodsListByColorDistinct(array('store_id'=>$value['store_id'], 'goods_state'=>1), $fieldstr, 'goods_salenum desc', 3); 
                //$store_array[$value['store_id']]['search_list_goods'] = $this->getHotSalesList($value['store_id'], 2); 
    	}
        //var_dump(json_encode($store_array));
    	return $store_array;
    }
}
