...
/////////////////////////////
 /**
     * 根据商品编号查询商品评价信息
     */
 public function getEvaluateGoodsInfoByOrderSn($order_sn) {
        $prefix = 'goods_evaluation';
        $info = rcache($goods_id, $prefix);
        if(empty($info)) {
            $info = array();
            $count_array = $this->field('count(*) as count,geval_scores')->where(array('geval_orderno' => $order_sn))->group('geval_scores')->key(geval_scores)->select();
		//	var_dump($count_array);exit;
           ...
           // $info['img'] = intval($this->where(array('geval_goodsid' => $goods_id, 'geval_image|geval_image_again' => array('neq', '')))->count());
            if(intval($info['all']) > 0) {
                $info['good_percent'] = intval($info['good'] / $info['all'] );
                $info['normal_percent'] = intval($info['normal'] / $info['all']);
                $info['bad_percent'] = intval($info['bad'] / $info['all']);
                $info['good_star'] = ceil($info['good'] / $info['all'] * 5);
                $info['star_average'] = ceil(($star1 + $star2 * 2 + $star3 * 3 + $star4 * 4 + $star5 * 5));
            } else {
                $info['good_percent'] = 100;
                $info['normal_percent'] = 0;
                $info['bad_percent'] = 0;
