...
class cartModel extends Model {
    ...
    private function _addCartDb($goods_info = array(), $quantity) {
        ...
        //检查购物车中:商品是否存在,存在则返回--购物车的信息.
        $check_cart = $this->checkCart($condition);
        //第二次加入购物车,执行下
        if (!empty($check_cart)) {
            $data['goods_num'] = $check_cart['goods_num'] + $quantity;
            Model()->query("update zmkj_cart set goods_num={$data['goods_num']} where cart_id={$check_cart['cart_id']}");
            return true;
        }
        //第一次加入购物车,执行:下
        $array = array();
    ...
    private function _addCartCookie($goods_info = array(), $quantity = null) {
        //去除斜杠
        $cart_str = get_magic_quotes_gpc() ? stripslashes(cookie('cart')) : cookie('cart'); //获取cookie值,并去掉斜杠.
        ...
        if (in_array($goods_info['goods_id'], array_keys($cart_array))) {
            $cart_array[$goods_info['goods_id']]['goods_num'] += $quantity;
    ...
    public function listCart($type, $condition = array(), $limit = '') {
        if ($type == 'db') {
            ...
            //去除斜杠
            $cart_str = get_magic_quotes_gpc() ? stripslashes(cookie('cart')) : cookie('cart'); //获取php.ini的配置,开启的话,则去掉/,关闭的话,则直接使用.
            $cart_str = base64_decode(decrypt($cart_str));
