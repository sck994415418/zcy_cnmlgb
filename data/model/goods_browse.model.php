...
class goods_browseModel extends Model {
    ...
    public function getViewedGoodsList($member_id = 0, $shownum = 0) {
        ...
        if ($browselist) {
            $goods_list_tmp = Model('goods')->getGoodsList(array('goods_id' => array('in', array_keys($browselist))), 'goods_id, goods_name, goods_promotion_price, goods_promotion_type, goods_marketprice, goods_image, store_id, gc_id, gc_id_1, gc_id_2, gc_id_3,vipprice,gjprice');
            $goods_list = array();
            foreach ((array) $goods_list_tmp as $key1 => $v) {
                $goods_list[$v['goods_id']] = $v;
                if ($_SESSION['mshenfen_id'] == 2 && $v['vipprice']!=0) {
                    $goods_list[$v['goods_id']]['price'] = $v['vipprice'] > $v['goods_promotion_price'] ? $v['goods_promotion_price'] : $v['vipprice'];
                } elseif ($_SESSION['mshenfen_id'] == 3 && $v['gjprice']!=0) {
                    $goods_list[$v['goods_id']]['price'] = $v['gjprice'] > $v['goods_promotion_price'] ? $v['goods_promotion_price'] : $v['gjprice'];
                } else {
                    $goods_list[$v['goods_id']]['price'] = $v['goods_promotion_price'];
                }
            }
            foreach ((array) $browselist as $k => $v) {
                if ($goods_list[$k]) {
                    $browselist_new[] = array(
                        ...
                        "goods_promotion_price" => $goods_list[$k]['price'],
						...
						"vipprice"          => $goods_list[$k]['vipprice'],
						"gjprice"          => $goods_list[$k]['gjprice'],
                        "store_id" => $goods_list[$k]['store_id']
                    );
                }
            }
        }
        //print_r($browselist_new);
