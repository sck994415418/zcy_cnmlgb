...
class goodsModel extends Model {
    ...
    public function getGoodsListByColorDistinct($condition, $field = '*', $order = 'goods_id asc', $page = 0) {
        ...
        //var_dump("dcolorcondition<br\>");
        //var_dump($condition);
        //var_dump("dcolorfiled");
        //var_dump($field);
        $goods_list = array();
        if ($count != 0) {
    ...
    public function getGoodsOnlineList($condition, $field = '*', $page = 0, $order = 'goods_id desc', $limit = 0, $group = '', $lock = false, $count = 0) {
        $condition['goods_state'] = self::STATE1;
        $condition['goods_verify'] = self::VERIFY1;
        //var_dump($this->getGoodsList($condition, $field, $group, $order, $limit, $page, $lock, $count));
     ...
     * @param string $order 排序:降序,最新添加,在最前面
     * @return array
     */
    ...
    private function _rGoodsImageCache($key) {
        return rcache($key, 'goods_image'); //读取图片路径缓存
    }
    ...
    public function getGoodsDetail($goods_id) {
        ...
        // 商品多图-先通过goods_id获取
        $image_more = Model()->query("SELECT goods_image FROM `zmkj_goods_images` WHERE goods_id = {$goods_info['goods_id']}  AND  color_id = {$goods_info['color_id']}  ORDER BY is_default desc,goods_image_sort asc LIMIT 1000");
        if (empty($image_more)) {
            // 商品多图-通过goods_commonid获取
	...
	public function getGoodeGoodsInfoByID($goods_id, $fields = '*') {
		
		$common_info = $this->getGoodeGoodsInfo(array('goods_id'=>$goods_id));    
		return $common_info;
    }
	
	public function getGoodeGoodsInfo($condition, $field = '*') {
        return $this->table('goods')->field($field)->where($condition)->find();
    }
