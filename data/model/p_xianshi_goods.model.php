...
class p_xianshi_goodsModel extends Model{
    ...
    public function getXianshiGoodsCommendList($count = 4) {
        ...
        $xianshi_list = $this->getXianshiGoodsExtendLists($condition, 18, 'xianshi_recommend desc', '*','goods_image', $count);
        return $xianshi_list;
    }
	...
	public function getXianshiGoodsExtendLists($condition, $page=null, $order='', $field='*',$group, $limit = 0) {
        $xianshi_goods_list = $this->getXianshiGoodsLists($condition, $page, $order, $field,$group, $limit);
        if(!empty($xianshi_goods_list)) {
            for($i=0, $j=count($xianshi_goods_list); $i < $j; $i++) {
                $xianshi_goods_list[$i] = $this->getXianshiGoodsExtendInfo($xianshi_goods_list[$i]);
            }
	...
	public function getXianshiGoodsLists($condition, $page=null, $order='', $field='*',$group, $limit = 0) {
        return $xianshi_goods_list = $this->field($field)->where($condition)->page($page)->order($order)->group($group)->limit($limit)->select();
	}
}
