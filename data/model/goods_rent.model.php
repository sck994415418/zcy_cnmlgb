...
class goods_rentModel extends Model{
    public function __construct(){
        parent::__construct('goods_rent');
    }
    ...
    public function addGoods($insert) {
        $result = $this->table('goods_rent')->insert($insert);
        if ($result) {
    ...
    public function addGoodsCommon($insert) {
        return $this->table('goods_rent')->insert($insert);
    }
    ...
    public function getGoodsList($condition, $field = '*', $group = '',$order = '', $limit = 0, $page = 0, $lock = false, $count = 0) {
        ...
        return $this->table('goods_rent')->field($field)->where($condition)->group($group)->order($order)->limit($limit)->page($page, $count)->lock($lock)->select();
    }
	 ...
	 * 
	 */
	 public function rent_goods_list($condition, $field = '*', $group = '',$order = 'rent_addtime desc', $limit = 0, $page = 20, $count = 0){
	 	$rent_goods_list = $this->table('goods_rent')->field($field)->where($condition)->group($group)->order($order)->limit($limit)->page($page, $count)->select();
//		var_dump($rent_goods_list);exit;
		return $rent_goods_list;
	 }
    ...
    public function getGoodsCommonList($condition, $field = '*', $page = 10, $order = 'goods_commonid desc') {
        $condition = $this->_getRecursiveClass($condition);
        return $this->table('goods_rent')->field($field)->where($condition)->order($order)->page($page)->select();
    }
    ...
    public function editGoodsById($update, $goodsid_array) {
        ...
        $result = $this->table('goods_rent')->where($condition)->update($update);
        if ($result) {
    ...
    public function getGoodsInfo($condition, $field = '*') {
        return $this->table('goods_rent')->field($field)->where($condition)->find();
    }
    ...
    public function getGoodsSum($condition, $field) {
        return $this->table('goods_rent')->where($condition)->sum($field);
    }
    ...
    public function getGoodsCount($condition) {
        return $this->table('goods_rent')->where($condition)->count();
    }
    ...
    public function getGoodsOnlineCount($condition, $field = '*') {
        $condition['goods_state']   = self::STATE1;
        $condition['goods_verify']  = self::VERIFY1;
        return $this->table('goods_rent')->where($condition)->group('')->count1($field);
    }
    /**
     * 获得商品数量
     *
    ...
    public function delGoods($condition) {
        ...
        return $this->table('goods_rent')->where($condition)->delete();
    }
	...
	/**
	 * 删除可租赁商品(租赁表中删除,商品不删除)
	 */
	public function delet_rent_goods($where){
//		var_dump($where);exit;
		$delete = $this->table('goods_rent')->field('*')->where($where)->delete();
//		var_dump($delete);exit;
    ...
    private function _rGoodsCache($goods_id, $fields) {
        return rcache($goods_id, 'goods_rent', $fields);
    }
    ...
    private function _wGoodsCache($goods_id, $goods_info) {
        return wcache($goods_id, $goods_info, 'goods_rent');
    }
    ...
    private function _dGoodsCache($goods_id) {
        return dcache($goods_id, 'goods_rent');
    }
    ...
    private function _rGoodsCommonCache($goods_commonid, $fields) {
        return rcache($goods_commonid, 'goods_rent', $fields);
    }
    ...
    private function _wGoodsCommonCache($goods_commonid, $common_info) {
        return wcache($goods_commonid, $common_info, 'goods_rent');
    }
    ...
    private function _dGoodsCommonCache($goods_commonid) {
        return dcache($goods_commonid, 'goods_rent');
    }
    ...
    private function _rGoodsSpecCache($goods_commonid) {
        return rcache($goods_commonid, 'goods_rent');
    }
    ...
    private function _wGoodsSpecCache($goods_commonid, $spec_list) {
        return wcache($goods_commonid, $spec_list, 'goods_rent');
    }
    ...
    public function getGoodsDetail($goods_id) {
        ...
        foreach ($spec_array as $key => $value) {
            ...
            $tpl_spec['url'] = urlShop('goods_rent', 'index', array('goods_id' => $value['goods_id']));
            $spec_list[] = $tpl_spec;
