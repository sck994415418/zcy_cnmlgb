...
class voucherModel extends Model {
    ...
    public function getCurrentAvailableVoucher($condition = array(), $goods_total = 0) {
       $condition['voucher_end_date'] = array('gt',TIMESTAMP); //代金券结束时间
       $condition['voucher_state'] = 1;//代金券状态，是否使用、过期等等
    ...
    public function getCanChangeTemplateInfo($vid,$member_id,$store_id = 0){
        ...
        //查询可用代金券模板
        $where = array();
        $where['voucher_t_id'] = $vid;//代金券模板id
        $where['voucher_t_state'] = $this->templatestate_arr['usable'][0];//代金券模板的状态
        $where['voucher_t_end_date'] = array('gt',time());//结束时间
    ...
    public function exchangeVoucher($template_info, $member_id, $member_name = ''){
        ...
        //2018年3月16日-赵廷建：增加开始
        $insert_arr['voucher_type'] = $template_info['voucher_t_type'];
        $insert_arr['voucher_type_id'] = $template_info['voucher_t_type_id'];
        //结束
