...
/**
     ...
     * @param string $key
     */
    public function getOrderGoodsList1($condition = array(), $fields = '*', $limit = null, $page = null, $order = 'rec_id desc', $group = null, $key = null) {
    	 $condition['order_state'] = array('in', array(ORDER_STATE_NEW,ORDER_STATE_PAY, ORDER_STATE_SEND, ORDER_STATE_SUCCESS));  		  
         return $this->table('order_goods,order')->join('inner')->on('order_goods.order_id=order.order_id')->
       // return $this->table('order_goods')->
        field($fields)->where($condition)->limit($limit)->order($order)->group($group)->key($key)->page($page)->select();
    }
/////////////////////////////////////////////	
...
///////////////////////////				
    	    ...
    	    case 'second_evaluation':
    	        $state = !$order_info['lock_state'] &&  ($order_info['evaluation_state'] == 3);
    	        break;
////////////////////////////
...
/*shop/control 控制器调用  开始*/
	////////////////////////////////////////	///////
	 /**
     * 评论记录 evaluation_state  0,1,2,3分别为 未评价,已评价,评价过期,追加评论
     * @param unknown $condition
     * @param string $field
     * @param number $page
     * @param string $order
     */
    public function getOrderAndOrderGoodSevaluationRecordList($condition, $field="*", $page = 0, $order = 'rec_id desc') {
        $condition['evaluation_state'] = array('in', array(1,3));		 
        return $this->getOrderAndOrderGoodsList($condition, $field, $page, $order);
    }
	/**
     * 订单销售记录(n) 订单状态为10、20、30、40时
     * @param unknown $condition
     * @param string $field
     * @param number $page
     * @param string $order   //order_state
     */
    public function getOrderNumAndOrderGoodsSalesRecordList($condition, $field="goods_num") {
        $condition['order_state'] = array('in', array(ORDER_STATE_NEW,ORDER_STATE_PAY, ORDER_STATE_SEND, ORDER_STATE_SUCCESS));  		  
         return $this->table('order_goods,order')->join('inner')->on('order_goods.order_id=order.order_id')->
         			where($condition)->field($field)->select();		 				         
    }
////////////////////////////////	
/*shop/control 控制器调用  结束*/
}
