...
<div class="home-standard-layout wrapper style-<?php echo $output['style_name']; ?>">
    ...
    <div class="left-sidebar">
        ...
        <?php
        if (!empty($output['code_recommends_list']['code_info']) && is_array($output['code_recommends_list']['code_info'])) {
            $i = 0;
            ?>
            <?php
            foreach ($output['code_recommends_list']['code_info'] as $key => $val) {
                $i++;
                ?>
            ...
            foreach ($output['code_recommend_list']['code_info'] as $key => $val) {
                ...
                if (!empty($val['goods_list']) && is_array($val['goods_list'])) {
                    foreach ($val['goods_list'] as $k => $v) {
                        ...
                        $sql = "select `goods_state` from zmkj_goods where `goods_id` = " . $v['goods_id'];
                        $state = Model()->query($sql);
                        if ($state[0]['goods_state'] !== '1') {
                            unset($val['goods_list'][$k]);
                        }
                    }
                    ?>
                            ...
                            <?php foreach ($val['goods_list'] as $k => $v) { ?>
                                <li>
                                    <dl>
                                        ...
                                        <dd class="goods-price"><em><?php
                                                $model_goods = Model('goods');
                                                $sql = "select goods_promotion_price,vipprice,gjprice from zmkj_goods where goods_id = " . $v['goods_id'];
                                                $price = $model_goods->query($sql);
                                                if ($_SESSION['mshenfen_id'] == 2 && $v['vipprice'] != 0) {
                                                    echo $price[0]['vipprice'] > $price[0]['goods_promotion_price'] ? $price[0]['goods_promotion_price'] : $price[0]['vipprice'];
                                                } elseif ($_SESSION['mshenfen_id'] == 3 && $v['gjprice'] != 0) {
                                                    echo $price[0]['gjprice'] > $price[0]['goods_promotion_price'] ? $price[0]['goods_promotion_price'] : $price[0]['gjprice'];
                                                } else {
                                                    echo $price[0]['goods_promotion_price'];
                                                }
                                                ?></em>
                                        </dd>
                                    </dl>
