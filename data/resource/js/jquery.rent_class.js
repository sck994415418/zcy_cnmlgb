$(document).ready(function(){
	...
	$('img[nc_type="flex"]').click(function(){
		...
		if(status == 'open'){
			...
			//ajax
			$.ajax({
				url: 'index.php?act=rent&op=rent_type&ajax=1&gc_parent_id='+id,
				dataType: 'json',
				success: function(data){
					...
					for(var i = 0; i < data.length; i++){
						...
						//新增下级
						if(data[i].deep < 3){
							src += "<a class='btn-add-nofloat marginleft' href='index.php?act=rent&op=type_add&gc_parent_id="+data[i].gc_id+"'><span>新增下级</span></a>";
						}
						src += "</td>";
						//类型
						...
						//操作
						src += "<td class='w84'>";
						src += "<a href='index.php?act=rent&op=type_edit&gc_id="+data[i].gc_id+"'>编辑</a>";
						src += " | <a href=\"javascript:if(confirm('删除该分类将会同时删除该分类的所有下级分类，您确定要删除吗'))window.location = 'index.php?act=rent&op=type_del&gc_id="+data[i].gc_id+"';\">删除</a>";
						src += "</td>";
						src += "</tr>";
					}
					//插入
					...
					$.getScript(RESOURCE_SITE_URL+"/js/jquery.rent_class.js");
