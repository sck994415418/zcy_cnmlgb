...
//         $this->_add_points();
    ...
    //被动绑定商品信息 查询
    public function ajax_is_showOp() {
        //下面的东西，应该放入model中。
        ...
        //print_r($goods_id);
        //获取商品信息:mysqli的拼接规则
        $array['field'] = "goods.goods_id,goods_bind.product_id";
        $array['table'] = " goods,goods_bind";
        $array['join_type'] = " left join";
        $array['join_on'][0] = " goods.goods_id=goods_bind.goods_id";
        $array['where'] = " goods.goods_id in ({$goods_id_str}) and goods_bind.product_id not like ''";
        $array['order'] = " goods.goods_id desc";

        $goods_orm_list = Db::select($array);
        $goods = "";
        foreach ($goods_orm_list as $value) {
            //echo $value['goods_id'];
            $url = "http://www.hebzfcgwssc.com/Mall/HeBei/detail.aspx?product_id=";
            $url .= $value['product_id'];
            $rs = $this->curl_get($url); //请求后匹配到的结果。
            if ($rs) {
                $goods .= $value['goods_id'] . " ";
                //如果能匹配到数据,则进行数据库的操作。
                $sql1 = "Update zmkj_goods_bind set is_show = 1 where goods_id='{$value['goods_id']}'";
    ...
    private function curl_get($url) {
        ...
        $pattern = "/shop-zoom-tt/";

        $rs = preg_match($pattern, $data);
        return $rs;
    }
