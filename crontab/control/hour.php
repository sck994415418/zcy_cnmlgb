...
class hourControl extends BaseCronControl {
    ...
    private function _ini_xs() {
        ...
        $this->_xs = new XS(C('fullindexer.appname')); //shopnc
    ...
    public function xs_createOp() {
        ...
        try {
            ...
            $count = Model()->query("select count(*) as num from zmkj_goods where goods_state=1 and goods_verify=1");
            //$count = $model_goods->getGoodsOnlineCount(array(),"distinct CONCAT(goods_commonid,',',color_id)");
            echo 'Total:' . $count[0]['num'] . "\n";
            //$fields = "*,CONCAT(goods_commonid,',',color_id) as nc_distinct";
            // for ($i = 0; $i <= $count; $i = $i + $step_num){
            //需要的字段：
            $goods_list = Model()->query("select * from zmkj_goods where goods_state=1 and goods_verify=1");
    ...
    public function _xs_update() {
        ...
        try {
            ...
            $model_goods = Model('goods');
            //$condition = array();
            $time=time()-$step_time;//定时5分钟，时间判断6分钟
            //$condition['goods_edittime'] = array('egt', TIMESTAMP - $step_time); //商品编辑时间：大于 等于
            //获取商品数量
            $count = Model()->query("select count(*) as num from zmkj_goods where goods_state=1 and goods_verify=1 and goods_edittime>={$time}");
            //$count = $model_goods->getGoodsOnlineCount($condition,"distinct CONCAT(goods_commonid,',',color_id)");
            //字段值
            //$fields = "*,CONCAT(goods_commonid,',',color_id) as nc_distinct";
            $goods_list = Model()->query("select * from zmkj_goods where goods_state=1 and goods_verify=1 and goods_edittime>={$time}");
            //for ($i = 0; $i <= $count; $i = $i + $step_num){
