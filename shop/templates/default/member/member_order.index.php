    ...
    <tbody order_id="" <?php if (!empty($group_info['pay_amount']) && $p == 0) {?> class="pay" <?php }?>>
      ...
      <tr>
        ...
        <td class="bdl bdr" rowspan="<?php echo $order_info['goods_count'];?>"><!-- 永久删除 -->
          ...
          <?php } ?>
           <!-- 追加评价 -->
            <?php if ($order_info['evaluation_state']==3) { ?>
          <p><a class="ncm-btn ncm-btn-acidblue" href="index.php?act=member_evaluate&op=second_add&order_id=<?php echo $order_info['order_id']; ?>"><i class="icon-thumbs-up-alt"></i><?php echo $lang['member_order_second_evaluate'];?></a></p>
          <?php } ?>  
        </td>
        <!-- E 合并TD -->
        <?php } ?>
