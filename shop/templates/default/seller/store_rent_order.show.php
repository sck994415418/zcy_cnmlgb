...
<div class="ncsc-oredr-show">
  <div class="ncsc-order-info">
    ...
    <div class="ncsc-order-condition">
      ...
      <ul>
        <li>1. 该笔订单的商品已由快递或物流发送至买家给出的地址。</li>
        <li>2. 本次交易从即日起至
    ...
    <table class="ncsc-default-table order">
      <thead>
        <tr>
          ...
          <th colspan="2">商品</th>
          <th class="w60">数量</th>
          <th class="w100">总租金 (元)</th>
          <th class="w100">总押金 (元)</th>
          <!--<th class="w60">押量 (元)</th>-->
          <th colspan="2" class="w100">交易状态</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <th colspan="20"><span class="ml10" title="虚拟订单号">虚拟订单号：<?php echo $output['order_info']['order_sn'];?></span><span>下单时间：<?php echo date("Y-m-d H:i",$output['order_info']['add_time']);?></span><span><span class="sale-type">订单租期：<?php echo date("Y-m-d",$output['order_info']['order_start'])?>起 至 <?php echo date("Y-m-d",$output['order_info']['order_end']);?></span></th>
        </tr>
        <tr>
          <td class="bdl"></td>
          ...
          <td><?php echo $output['order_info']['order_amount'];?></td>
          <td><?php echo $output['order_info']['order_pledge'];?></td>
          <td class="bdl"><?php if($output['order_info']['order_state'] == 0){echo '已取消';} else if($output['order_info']['order_state'] == 10){echo '未付款';} else if($output['order_info']['order_state'] == 20){echo '已付款';} else if($output['order_info']['order_state'] == 30){echo '已发货';} else if($output['order_info']['order_state'] == 40){echo '已收货';} else if($output['order_info']['order_state'] == 50){echo '租赁中';} ?></td>
        </tr>
      </tbody>
      <tfoot>
