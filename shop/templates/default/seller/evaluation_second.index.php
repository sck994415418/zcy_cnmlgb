...
<div class="tabmenu">
    <ul id="listpj" class="tab">
        ...
        <li class="active"><a href="<?php echo urlShop('store_evaluate_second', 'list'); ?>"><?php echo $lang['member_second_evaluation_frombuyer']; ?></a></li>
    </ul>
</div>
<form method="get">
            ...
            <?php foreach ((array) $output['goodsevallist'] as $k => $v) { ?>
                <tr>
                    ...
                    <th>
                        ...
                        <span>评价人：<?php echo $v['geval_frommembername']; ?> <time>[<?php echo date('Y-m-d H:i:s', $v['geval_second_addtime']); ?>]</time></span>
                    </th>
                    <th></th>
                </tr>
                <tr>
                    ...
                    <td class="tl">
                        <strong>评价内容：</strong>
                        <span><?php echo $v['geval_second_content']; ?></span>
                    </td>
                    <td rowspan="2" class="nscs-table-handle vt">
                        <span><a nctype="btn_show_explain_dialog" data-geval-id="<?php echo $v['geval_id']; ?>" data-geval-content="<?php echo $v['geval_second_content']; ?>" href="javascript:;" class="btn-acidblue">
                ...
                <tr class="bd-line" >
                    <td class="tl" colspan="20">
                        <div <?php echo empty($v['geval_second_explain']) ? 'style="display:none;"' : '' ?>>
                            <strong>解释内容：</strong>
                            <span nctype="explain"><?php echo $v['geval_second_explain']; ?></span>
                        </div>
                    </td>
                </tr>
            <?php } ?>
...
<div id="dialog_explain" style="display:none;">
    <div class="eject_con">
        ...
        <form id="explain_form">
            ...
            <dl>
                <dt>评价内容：</dt>
                <dd id="geval_second_content"></dd>
            </dl>
            <dl>
                <dt><i class="required">*</i>解释内容：</dt>
                <dd>
                    <textarea id="geval_second_explain" name="geval_second_explain" cols="30" rows="10"></textarea>
                </dd>
            </dl>
            <div class="bottom">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('[nctype="btn_show_explain_dialog"]').on('click', function () {
            ...
            var geval_second_content = $(this).attr('data-geval-content');
            $('#geval_id').val(geval_id);
            $('#geval_second_content').text(geval_second_content);
            $('#geval_second_explain').val('');
...
//给按钮绑定“点击”事件，然后a
        $('#btn_explain_submit').on('click', function () {
            if ($('#explain_form').valid()) {
                ...
                var geval_second_explain = $('#geval_second_explain').val();
                $.post("<?php echo urlShop('store_evaluate_second', 'explain_save'); ?>", {
                    geval_id: geval_id,
                    geval_second_explain: geval_second_explain
                }, function (data) {
                    if (data.result) {
                        $('#dialog_explain').hide();
                        $item.text(geval_second_explain);
                        $item.parent().show();
