...
<div class="tabmenu">
  ...
  <a href="<?php echo urlShop('store_rent_order', 'exchange');?>" class="ncsc-btn ncsc-btn-orange"><i class="icon-edit"></i>兑换兑换码</a>
</div>
<form method="get" action="index.php" target="_self">
  <table class="search-form">
    <input type="hidden" name="act" value="store_rent_order" />
...
<table class="ncsc-default-table order">
  ...
  <?php foreach($output['order_list'] as $rorder_id => $order) { ?>
  <tbody>
    <tr>
...
<?php } ?>
      ...
      <td class="bdl bdr"><p><?php echo $order['state_desc']; ?>
        ...
        <!-- 订单查看 -->
        <p><a href="index.php?act=store_rent_order&op=show_order&rorder_id=<?php echo $order['rorder_id'];?>" target="_blank"><?php echo $lang['store_order_view_order'];?></a></p>
        </td>
      <td class="bdl bdr">
      <!-- 取消订单 -->
        <?php if($order['if_cancel']) { ?>
        <p><a href="javascript:void(0)" class="ncsc-btn ncsc-btn-red mt5" nc_type="dialog" uri="index.php?act=store_rent_order&op=change_state&state_type=order_cancel&order_sn=<?php echo $order['order_sn']; ?>&rorder_id=<?php echo $order['rorder_id']; ?>" dialog_title="<?php echo $lang['store_order_cancel_order'];?>" dialog_id="seller_order_cancel_order" dialog_width="400" id="order<?php echo $order['rorder_id']; ?>_action_cancel" /><i class="icon-remove-circle"></i><?php echo $lang['store_order_cancel_order'];?></a></p>
        <?php } ?>
    </tr>
    <?php } } else { ?>
    <tr>
