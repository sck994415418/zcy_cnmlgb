            ...
            <div class="content">
                <dl class="focus">
                    ...
                    <dd title="已发布/<?php echo $lang['store_publish_goods']; ?>"><em id="nc_goodscount">0</em>&nbsp;/&nbsp;
                        <?php
                        if ($output['store_info']['grade_goodslimit'] != 0) {
                            echo $output['store_info']['grade_goodslimit'];
                        } else {
                            echo '不限';
                        }
                        ?>
                ...
                <table class="ncsc-default-table">
                    ...
                    <tbody>
                        <?php if ($output['goods_list']) { ?>
                            <?php
                            $i = 0;
                            foreach ($output['goods_list'] as $val) {
                                $i++
                                ?>
                                <tr class="bd-line">
...
<!--<link href="http://www.nrwspt.com/shop/templates/default/easyui/themes/default/easyui.css" rel="stylesheet" type="text/css"/>
<link href="http://www.nrwspt.com/shop/templates/default/easyui/themes/icon.css" rel="stylesheet" type="text/css"/>
<link href="http://www.nrwspt.com/shop/templates/default/easyui/demo/demo.css" rel="stylesheet" type="text/css"/>
<script src="http://www.nrwspt.com/shop/templates/default/easyui/jquery.easyui.min.js" type="text/javascript"></script>
<style type="text/css">
...
<script>
    //必须是“新订单”，条件：数量大于0，且大于“旧订单数量”
    $(function () {
        odr1();
        $('#dlg').dialog('close');
        setInterval("odr2()", 5000); //分钟刷新一次
    })
    function odr1() {
    ...
    function odr2() {
        ...
        $.getJSON('index.php?act=seller_center&op=statistics&rand=' + timestamp, null, function (data) {
            ...
            var _old_num = parseInt($("#nc_delivery").html());
            var _new_num = data['delivery'];
            //新订单的数量      
    ...
    //铃声：执行n次，回调函数
    function _ado(_i) {
        var audio = new Audio('http://www.nrwspt.com/shop/templates/default/audio/2823.mp3');
        audio.play();
        _i++;
        //关闭“小窗口”停止执行;或者执行“30”次。
        if (!($('.panel.window.panel-htop').css("display") == "none" || _i == 30)) {
            setTimeout("_ado(" + _i + ")", 1000);
    ...
    //字体颜色
    function _oder() {
        var _color = Math.ceil(Math.random() * 6);
        switch (_color) {
            case 1:
...
<div id="dlg" class="easyui-dialog" title="订单提醒" data-options="iconCls:'icon-save'" style="height:auto;padding:10px;background:#00ccff">
    <span id='odr_show' style='font-size:20px'>老板，来订单了！</span>
     <h1 id='odr_show' style="font-size:30px;font-family: '华文行楷';color:#000000;line-height:54px;text-align:center">老板，来活了！</h1>
</div>-->
