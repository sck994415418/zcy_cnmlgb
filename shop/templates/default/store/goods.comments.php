...
<?php if(!empty($output['goodscommon']) && is_array($output['goodscommon'])){?>
<?php foreach($output['goodscommon'] as $k=>$v){?>
...
<!--照片-->    	
    ...
    <!--晒单图片：-->
       ...
       <!-- <li><a nctype="nyroModal"  href="<?php echo snsThumb($value, 1024);?>">
        ...
        <?php } ?></ul>
        <!--一张图片
        <img src="<?php echo $v['geval_image'];?>" width="150px" height="150px">-->
    </dd>
    <?php } ?>
<!--照片--> 
<!--追加评论-->
<?php if(!empty($v['geval_second_content'])){?>
	<dd class="content">[<?php echo @date('Y-m-d',$v['geval_second_addtime']);?>]&nbsp;&nbsp;&nbsp;&nbsp;追加评论：<span><?php echo $v['geval_second_content'];?></span></dd>
    <?php if (!empty($v['geval_explain'])){?>
    <dd class="explain"><?php echo $lang['nc_credit_explain'];?>：<span><?php echo $v['geval_explain'];?></span></dd>
    <?php } ?>
    <?php }?>
...
<div class="tr pr5 pb5 pr">
  <a href="" target="_blank" class="more-commend"></a><div class="pagination"> <?php echo $output['show_page'];?></div>
</div>
<?php }else{?>
