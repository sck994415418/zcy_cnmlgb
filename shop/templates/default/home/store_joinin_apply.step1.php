...
<div id="apply_company_info" class="apply-company-info">
  <div class="alert">
    <h4>注意事项：</h4>
    以下所需要上传的电子版资质文件仅支持JPG\GIF\PNG格式图片，大小请控制在2M之内。</div>
  <form id="form_company_info" action="index.php?act=store_joinin&op=step2" method="post" enctype="multipart/form-data" >
	...
	<!--V3-B10简化 -->
        ...
        <tr>
          <th><i>*</i>联系人手机：</th>
          <td><input name="contacts_phone" type="tel" onkeyup="value=value.replace(/[^\d]/g,'')" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))" class="w100" />
            <span></span></td>
        </tr>
        <tr>
          <th><i>*</i>电子邮箱：</th>
        ...
        <tr>
          <th><i>&nbsp;</i>融云客服id：</th>
          <td>
            <input name="serviceid" type="text" class="w200" />
            <span></span>
          </td>
        </tr>
      </tbody>
    ...
    <table border="0" cellpadding="0" cellspacing="0" class="all">
      ...
      <tbody>
        ...
        <tr>
          <th><i>*</i>营业执照电子版：</th>
          <td><input name="business_licence_number_electronic" id="business_licence_number_electronic" type="file" class="w200" />
            <span class="block">图片大小请控制在2M之内，请确保图片清晰，文字可辨并有清晰的红色公章。</span></td>
            <!--<p class="none" style="color: red;"></p>-->
        </tr>
      </tbody>
      <tfoot>
        <tr>
    ...
    <table border="0" cellpadding="0" cellspacing="0" class="all">
      ...
      <tbody>
        ...
        <tr>
          <th>组织机构代码证电子版：</th>
          <td><input name="organization_code_electronic" id="organization_code_electronic" type="file" />
...
<script src="D:\www\html-1\shop\resource\js\jquery-2.2.0.js"></script>
...
$(document).ready(function(){
    ...
    $('#btn_apply_company_next').on('click', function() {
	        ...
	        var images1 = document.getElementById('business_licence_number_electronic').files[0];
	        var images2 = document.getElementById('organization_code_electronic').files[0];
					if(images1.size > 2*1024*1024 || images2.size > 2*1024*1024) {
						alert('上传图片不能大于2M,请重新选择');
//							document.getElementsByClassName('none').HTML('图片不能大于2M,请重新选择');
						return false;
					}
