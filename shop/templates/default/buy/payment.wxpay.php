...
$(document).ready(function(){
	window.queryOrderState = function (){
		$.ajax({
			type: "GET",
			...
			success: function(result) {
				if(result.state==1){
					if (result.type == 'r') {
						...
						window.location.href = '<?php echo SHOP_SITE_URL.'/index.php?act=predeposit&op=index'?>';
					} else {
	...
	setTimeout(window.queryOrderState,3000);
	setTimeout(function(){
		window.queryOrderState();
	},3000);
});
