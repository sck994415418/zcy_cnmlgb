...
<div class="ncc-receipt-info">
    ...
    <table class="ncc-table-style">
        ...
        <?php foreach ($output['store'] as $store_id => $brand) { ?>
            <tbody>
                <tr>
                    <th colspan="20"><strong>店铺：<a href="<?php echo urlShop('show_store', 'index', array('store_id' => $store_id)); ?>"><?php
                                foreach ($brand as $brand_id => $goods) {
                                    $i = 1;
                                    if ($i = 1) {
                                        $i++;
                                        echo $goods[0]['store_name'];
                                    }
                                }
                                ?></a></strong> <span member_id="<?php echo $output['store_list'][$store_id]['member_id']; ?>"></span>
                ...
                <?php foreach ($brand as $brand_id => $goods_list) { ?>
                    <!--一组商品：多行-->  
                    <?php foreach ($goods_list as $goods) { ?>
                        <!--一个商品：一行-->
                        <tr id="cart_item_<?php echo $goods['cart_id']; ?>" class="shop-list <?php echo ($goods['state'] && $goods['storage_state']) ? '' : 'item_disabled'; ?>">
                            <!--第一个td:hidden-->
                            <td><?php if ($goods['state'] && $goods['storage_state']) { ?>
                                    <!--购物车id/商品数量-->
                                    <input type="hidden" value="<?php echo $goods['cart_id'] . '|' . $goods['goods_num']; ?>" name="cart_id[]">
                                <?php } ?>
                            </td>
                            <!--第一个td:是否是套装-->  
                            <?php if ($goods['bl_id'] == '0') { ?>
                                <td class="w60"><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $goods['goods_id'])); ?>" target="_blank" class="ncc-goods-thumb"><img src="<?php echo thumb($goods, 60); ?>" alt="<?php echo $goods['goods_name']; ?>" /></a></td>
                            <?php } ?>
                            <!--第一个td:活动--> 
                            <td class="tl" <?php if ($goods['bl_id'] != '0') { ?>colspan="2"<?php } ?>>
                                <dl class="ncc-goods-info">
                                    <dt><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $goods['goods_id'])); ?>" target="_blank"><?php echo $goods['goods_name']; ?></a></dt>
                                    <?php if (!empty($goods['xianshi_info'])) { ?>
                                        <dd> <span class="xianshi">满<strong><?php echo $goods['xianshi_info']['lower_limit']; ?></strong>件，单价直降<em>￥<?php echo $goods['xianshi_info']['down_price']; ?></em></span> </dd>
                                    <?php } ?>
                                    <?php if ($goods['ifgroupbuy']) { ?>
                                        <dd> <span class="groupbuy">抢购</span></dd>
                                    <?php } ?>
                                    <?php if ($goods['bl_id'] != '0') { ?>
                                        <dd> <span class="buldling">优惠套装，单套直降<em>￥<?php echo $goods['down_price']; ?></em></span></dd>
                                    <?php } ?>
                                        ...
                                        <dd><span class="ncc-goods-gift">赠</span>
                                            <ul class="ncc-goods-gift-list">
                                                <?php foreach ($goods['gift_list'] as $goods_info) { ?>
                                                    <li nc_group="<?php echo $goods['cart_id']; ?>"><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $goods_info['gift_goodsid'])); ?>" target="_blank" class="thumb" title="赠品：<?php echo $goods_info['gift_goodsname']; ?> * <?php echo $goods_info['gift_amount'] * $goods['goods_num']; ?>"><img src="<?php echo cthumb($goods_info['gift_goodsimage'], 60, $store_id); ?>" alt="<?php echo $goods_info['gift_goodsname']; ?>"/></a> </li>
                                                <?php } ?>
                                            </ul>
                                        </dd>
                                    <?php } ?>
                            ...
                            <td class="w120"><em><?php echo $goods['goods_price']; ?></em></td>
                            <td class="w60"><?php echo $goods['state'] ? $goods['goods_num'] : ''; ?></td>
                            <td class="w120"><?php if ($goods['state'] && $goods['storage_state']) { ?>
                                    <em id="item<?php echo $goods['cart_id']; ?>_subtotal" nc_type="eachGoodsTotal"><?php echo $goods['goods_total']; ?></em>
                                <?php } elseif (!$goods['storage_state']) { ?>
                                    <span style="color: #F00;">库存不足</span>
                                <?php } elseif (!$goods['state']) { ?>
                        ...
                        <?php if (is_array($goods['bl_goods_list'])) { ?>
                            <?php foreach ($goods['bl_goods_list'] as $goods_info) { ?>
                                <tr class="shop-list <?php echo $goods['state'] && $goods['storage_state'] ? '' : 'item_disabled'; ?>">
                                    <td></td>
                ...
                <script type="text/javascript">
                    //全品类代金券和品牌代金券选择
                    function choose_all_or_brand(_js_obj) {
                        var _obj = $(_js_obj);
                        if (_obj.attr('class') == "voucher_brand") {
                            //品牌类代金券
                            var _brand_value = _obj.val();
                            var _voucher_brand_price = parseFloat((_brand_value.split("|"))[2]);
                            if (_voucher_brand_price !=0.00) {
                                var _voucher_all_object = $(".voucher_brand").parents('tbody').find('.voucher_all');
                                _voucher_all_object.attr("disabled", 'true');
                                return;
                            } else {
                                var _voucher_all_object = $(".voucher_brand").parents('tbody').find('.voucher_all');
                                _voucher_all_object.removeAttr("disabled");
                                return;
                            }
                        } else if (_obj.attr('class') == "voucher_all") {
                            //全品类代金券
                            var _all_value = _obj.val();
                            var _voucher_all_price = parseFloat((_all_value.split("|"))[2]);
                            if (_voucher_all_price !=0.00) {
                                var _voucher_brand_object = $(".voucher_all").parents('tbody').find('.voucher_brand');
                                _voucher_brand_object.attr("disabled", 'true');
                                return;
                            } else {
                                var _voucher_brand_object = $(".voucher_all").parents('tbody').find('.voucher_brand');
                                _voucher_brand_object.removeAttr("disabled");
                                return;
                            }
                        }
                    }
                </script>
                <?php if (!empty($output['store_brand_voucher'][$store_id][$brand_id]) && is_array($output['store_brand_voucher'][$store_id][$brand_id])) { ?>
                    <tr>
                        <td colspan="6">    
                            <dl class="voucher" style="overflow: hidden">
                                <dd style="float:right;width:108px;height:28px;margin-right:5px;color:#393"><em class="eachStoreVoucher" id="eachStoreVoucher_<?php echo $store_id; ?>">-0.00</em><?php echo $lang['currency_zh']; ?></dd>
                                <dt style="float:right;margin-right:6px">
                                    <!--代金券id/店铺id/代金券面额-->
                                    <select class="voucher_brand" onchange="javascript:choose_all_or_brand(this);" nctype="voucher" name="voucher[<?php echo $store_id; ?>][<?php echo $brand_id; ?>]">
                                        <option value="<?php echo $voucher['voucher_t_id']; ?>|<?php echo $store_id; ?>|0.00">选择代金券</option>
                                        <?php foreach ($output['store_brand_voucher'][$store_id][$brand_id] as $voucher_brand) { ?>
                                            <option value="<?php echo $voucher_brand['voucher_t_id']; ?>|<?php echo $store_id; ?>|<?php echo $voucher_brand['voucher_price']; ?>"><?php echo $voucher_brand['desc']; ?></option>
                                        <?php } ?>
                                    </select>
                                </dt>  
                            </dl>
                ...
                <td class="tr" colspan="20">
                    <div class="ncc-store-account">
                        ...
                        <?php if (!empty($output['store_voucher_list'][$store_id]) && is_array($output['store_voucher_list'][$store_id])) { ?>
                            <dl class="voucher">
                                <dt>
                                    <select class="voucher_all" onchange="javascript:choose_all_or_brand(this);"  nctype="voucher" name="voucher[<?php echo $store_id; ?>][0]">
                                        <option value="<?php echo $voucher['voucher_t_id']; ?>|<?php echo $store_id; ?>|0.00">选择代金券</option>
                                ...
                                <dd><em class="eachStoreVoucher" id="eachStoreVoucher_<?php echo $store_id; ?>">-0.00</em><?php echo $lang['currency_zh']; ?></dd>
                            </dl>
                        <?php } ?>
                        ...
                        <?php if (!empty($output['available_pd_amount'])) { ?>
                            <div class="mt5 mb5">
                                <label>
                                    ...
                                    使用预存款（可用金额：<em><?php echo $output['available_pd_amount']; ?></em><?php echo $lang['currency_zh']; ?>）
                                </label>
                            </div>
                        <?php } ?>
                        <?php if (!empty($output['available_pd_amount']) && !empty($output['available_rcb_amount'])) { ?>
