    ...
    function calcOrder() {
        ...
        $('em[nc_type="eachStoreTotal"]').each(function () {
            ...
            if ($('#eachStoreFreight_' + store_id).length > 0) {
                //console.log("运费:" + $('#eachStoreFreight_' + store_id).html());
            ...
            if ($(this).parents('tbody').find('.eachStoreVoucher').length > 0) {
                $(this).parents('tbody').find('.eachStoreVoucher').each(function () {
                    eachTotal += parseFloat($(this).html());
                });
            }
//            console.log("代金券:"+$(this).parents('tbody').find('.eachStoreVoucher').html());
//            if ($('#eachStoreVoucher_' + store_id).length > 0) {
//                //console.log($('#eachStoreVoucher_' + store_id));
...
//            }
    ...
    $(function () {
        //为将来的ajax:默认设置
        //ajax:true,异步的,请求的时候,不会锁死;false,同步的,请求的时候,会将浏览器锁死
        $.ajaxSetup({
            async: false
        });
        //绑定事件
        $('select[nctype="voucher"]').on('change', function () {
            if ($(this).val() == '') {
                ...
                $(this).parents("dl.voucher").find(".eachStoreVoucher").html('-' + number_format(items[2], 2));
            }
            calcOrder();
        });
