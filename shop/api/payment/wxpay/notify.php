...
class PayNotifyCallBack extends WxPayNotify
{
	...
	public function NotifyProcess($data, &$msg)
	{
		...
		if (in_array($data['attach'],array('v','r','p'))) {
		    $order_type = $data['attach'];
		} else {
		    $order_pay_info = Model('order')->getOrderPayInfo(array('pay_sn'=> $data['out_trade_no']));
		    if(empty($order_pay_info)){
				$order_pay_info = Model('predeposit')->getPdRechargeInfo(array('pdr_sn'=> $data['out_trade_no']));
				if(empty($order_pay_info)){
					$order_type = 'v';
				}else{
					$order_type = 'p';
		...
		if ($order_type == 'r') {
			...
			$order_pay_info = Model('predeposit')->getPdRechargeInfo(array('pdr_sn'=> $data['out_trade_no']));
		    if (!in_array($result['data']['pdr_payment_state'],array(0,1))) {
		        return true;
		    }
		    $order_info = $result['data'];			
		    $result = $logic_payment->updatePdOrder($data['out_trade_no'], $data['out_trade_no'], $order_info, $order_pay_info);
		    if (!$result) {
		        return false;
		    }
