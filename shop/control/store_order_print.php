		...
		if (!empty($order_info['extend_order_goods'])){
			...
			foreach ($order_info['extend_order_goods'] as $k => $v){
				if(!empty($v['goods_id'])){
					...
					$a=Model()->query("select vipprice,gjprice from zmkj_goods where goods_id=".$v['goods_id']);
					if ($member_id[0]['mshenfen_id'] == 2 && $a[0]['vipprice'] != '0.00'){
						$v['goods_price'] = $a[0]['vipprice'];
						$v['goods_pay_price'] = $a[0]['vipprice']*$v['goods_num'];
					}elseif($member_id[0]['mshenfen_id'] == 3 && $a[0]['gjprice'] != '0.00'){
						$v['goods_price'] = $a[0]['gjprice'];
						$v['goods_pay_price'] = $a[0]['vipprice']*$v['goods_num'];
					} 
				}
