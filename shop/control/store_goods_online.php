...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function edit_goodsOp() {
        ...
        Tpl::output('sign_i', count($spec_list)); //规格的数量
        Tpl::output('spec_list', $spec_list); //规格的列表
        Tpl::output('attr_list', $attr_list); //取得基本的属性值，和下面共同作用，选中状态。
        Tpl::output('brand_list', $brand_list);
        // 取得商品规格的输入值
        $goods_array = $model_goods->getGoodsList($where, 'goods_id,goods_commonid,goods_marketprice,goods_price,goods_storage,goods_serial,goods_storage_alarm,goods_spec,vipprice,gjprice');
        $sp_value = array();
        if (is_array($goods_array) && !empty($goods_array)) {
            // 取得已选择了哪些商品的属性:通过goods_id查询goods_attr_index
            ...
            Tpl::output('attr_checked', $attr_checked); //下路框的选择项。
            //由goods_commonid来查询goods_common表。 
            $goods_attr = Model()->query("select goods_attr from zmkj_goods_common where goods_commonid={$goods_array[0]['goods_commonid']}");
            $goods_attr = unserialize($goods_attr[0]['goods_attr']);
            //根据键来查询数据库attribute
            foreach ($goods_attr as $key1 => $value1) {
                $goods_attr_value = Model()->query("select attr_value from zmkj_attribute where attr_id={$key1}");
                if (empty($goods_attr_value[0]['attr_value'])) {
                    foreach ($value1 as $key2 => $value2) {
                        if (is_int($key2)) {
                            $goods_common_attr[$key1] = $value2;
                        }
                    }
                }
            }
            Tpl::output('goods_common_attr', $goods_common_attr);
            $spec_checked = array();
            foreach ($goods_array as $k => $v) {
                ...
                if (!empty($a)) {
                    ...
                    $sp_value ['i_' . $id . '|vipprice'] = $v['vipprice'];
                    $sp_value ['i_' . $id . '|gjprice'] = $v['gjprice'];
        ...
        Tpl::output('goods_handle_type', "修改商品名称,请点击<span style='color:red;font-weight:bold'>编辑</span>按钮"); //编辑商品状态
    ...
    public function edit_save_goodsOp() {
        ...
        foreach ($attr as $key => $val) {
            if (in_array("", $val) || in_array(null, $val)) {
                unset($attr[$key]);
            }
        }
        $update_common['goods_attr'] = serialize($attr); //二维数组[属性name_id][属性_value_id]，保存在goods_common表中
        ...
        $update_common['vipprice'] = floatval($_POST['g_vip_price']);
        $update_common['gjprice'] = floatval($_POST['g_gj_price']);
        ...
        if (is_array($_POST ['spec'])) { //有规格
            foreach ($_POST['spec'] as $value) {
                ...
                if (!empty($goods_info)) {
                    ...
                    $update['vipprice'] = $value['vipprice'];
                    $update['gjprice'] = $value['gjprice'];
                    ...
                    $insert['vipprice'] = $value['vipprice'];
                    $insert['gjprice'] = $value['gjprice'];
            ...
            if (!empty($goods_info)) {
                ...
                $update['vipprice'] = $update_common['vipprice'];
                $update['gjprice'] = $update_common['gjprice'];
                ...
                $insert['vipprice'] = $update_common['vipprice'];
                $insert['gjprice'] = $update_common['gjprice'];
    ...
    public function edit_imageOp() {
        ...
        Tpl::output('edit_goods_sign', true);
        $model_goods = Model('goods');
        $img_array = $model_goods->getGoodsList(array('goods_commonid' => $common_id), 'goods_id,goods_name,goods_spec,color_id,goods_image', 'color_id');
        $goods_list = Model()->query("select goods_name,goods_id from zmkj_goods where goods_commonid={$common_id}");
        Tpl::output('goods_info1', $goods_list);
	...
	public function edit_goods_imageOp(){
		$goods_id = intval($_GET['goods_id']);
        if ($goods_id <= 0) {
            showMessage(L('wrong_argument'), urlShop('seller_center'), 'html', 'error');
        }
		$model_goods = Model('goods');
        $common_list = $model_goods->getGoodeGoodsInfoByID($goods_id, 'store_id,goods_lock,spec_value,is_virtual,is_fcode,is_presell');
		if ($common_list['store_id'] != $_SESSION['store_id'] || $common_list['goods_lock'] == 1) {
            showMessage(L('wrong_argument'), urlShop('seller_center'), 'html', 'error');
        }
		$spec_value = unserialize($common_list['spec_value']);
		...
		$image_list = $model_goods->getGoodsImageList(array('goods_id' => $goods_id));
        $image_list = array_under_reset($image_list, 'color_id', 2);
		
		$img_array = $model_goods->getGoodsList(array('goods_id' => $goods_id), 'color_id,goods_image', 'color_id');
		...
		Tpl::output('common_list', $common_list);
		
        Tpl::output('goods_id', $goods_id);

        Tpl::showpage('store_goods_image');
		
	}
	/**
     * 保存商品图片
     */
    public function save_goods_imageOp() {
        if (chksubmit()) {
			$goods_id = intval($_POST['goods_id']);
			if ($goods_id <= 0 || empty($_POST['img'])) {
				showDialog(L('nc_common_save_fail'), urlShop('store_goods_online', 'edit_goods_image',array('goods_id'=>$goods_id)));
            }

			$model_goods = Model('goods');
            // 删除原有图片信息
            $model_goods->delGoodsImages(array('goods_id' => $goods_id, 'store_id' => $_SESSION['store_id']));
            // 保存
            $insert_array = array();
            foreach ($_POST['img'] as $key => $value) {
                foreach ($value as $v) {
                    ...
                    $update_where = array();
                    $update_array['goods_image'] = $v['name'];
                    $update_where['goods_id'] = $goods_id;
                    $update_where['store_id'] = $_SESSION['store_id'];
                    $update_where['color_id'] = $key;
                    if ($k == 0 || $v['default'] == 1) {
                        $k++;
