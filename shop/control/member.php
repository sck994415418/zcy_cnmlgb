...
class memberControl extends BaseMemberControl{
    ...
    public function ajax_load_goods_infoOp() {
        ...
        if (!empty($favorites_list) && is_array($favorites_list)){
            ...
            $field = 'goods.goods_id,goods.goods_name,goods.store_id,goods.goods_image,goods.goods_price,goods.evaluation_count,goods.goods_salenum,goods.goods_collect,store.store_name,store.member_id,store.member_name,store.store_qq,store.store_ww,store.store_domain,vipprice,gjprice';
            ...
            if (!empty($goods_list) && is_array($goods_list)){
                ...
                foreach ($goods_list as $key=>$fav){
                    $fav_id = $fav['goods_id'];
					//0510 miaojia 
					if ($_SESSION['mshenfen_id'] == 2 && $fav['vipprice'] != '0.00'){
						$fav['goods_price'] = $fav['vipprice'];
					}elseif($_SESSION['mshenfen_id'] == 3 && $fav['gjprice'] != '0.00'){
						$fav['goods_price'] = $fav['gjprice'];
					} 
    ...
    public function ajax_load_sns_infoOp() {
        ...
        if(is_array($goods_list) && !empty($goods_list)) {
            foreach ($goods_list as $key => $val) {
				...
				//0510 miao
				if ($_SESSION['mshenfen_id'] == 2 && $val['vipprice'] != '0.00'){
					$val['goods_promotion_price'] = $val['vipprice'];
				}elseif($_SESSION['mshenfen_id'] == 2 && $val['gjprice'] != '0.00'){
					$val['goods_promotion_price'] = $val['gjprice'];
				} 
