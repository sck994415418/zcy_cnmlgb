	...
	public function changPriceOp(){
		if($_POST["goods_id"]!="" and $_POST["new_price"]!=""){
			...
			if($rs=$zf->select_data($sql)){
				...
				$sql="update `zmkj_goods` set `goods_marketprice` = $goods_marketprice , `goods_promotion_price`=$new_price, `goods_edittime`= unix_timestamp(now()) where `store_id` = ".$_SESSION['store_id']." and  `goods_id` = $goods_id";
...
/*        $where = array();
        ...
        switch ($_GET['type']) {
            case 'change_all':// 商品批量改价
                Tpl::showpage('store_goods_list.change_price_chang_all');
                break;
