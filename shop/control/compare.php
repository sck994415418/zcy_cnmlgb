...
class compareControl extends BaseHomeControl{
        ...
        if($gid_arr){
            ...
            if($goods_list && $gcommon_list){
                foreach($goods_list as $k=>$v){
					$goods_commonids =$goods_list[$k]['goods_commonid'];
					$gcommon_list[$goods_commonids]['vipprice']=$goods_list[$k]['vipprice'];
					$gcommon_list[$goods_commonids]['gjprice']=$goods_list[$k]['gjprice'];
            ...
            for ($j=0; $j<self::MAXNUM; $j++){
				//0510 miao
				if ($_SESSION['mshenfen_id'] == 2 && $compare_listtmp[$j]['vipprice'] != '0.00'){
					$compare_list[$i][] = $compare_listtmp[$j]?$compare_listtmp[$j]['vipprice']:'';
				}elseif($_SESSION['mshenfen_id'] == 2 && $compare_listtmp[$j]['gjprice'] != '0.00'){
					$compare_list[$i][] = $compare_listtmp[$j]?$compare_listtmp[$j]['gjprice']:'';
				}else{
    ...
    public function showcompareOp(){
        ...
        if($gid_arr){
            ...
            $goods_list = array();
            $goodsid_arr = array();
            $goods_listtmp = $model->getGoodsList(array('goods_id'=>array('in',$gid_arr),'goods_state'=>1,'goods_verify'=>1),'goods_id,goods_commonid,goods_name,store_id,gc_id,gc_id_1,gc_id_2,gc_id_3,goods_promotion_price,goods_promotion_type,goods_image,vipprice,gjprice');
            if ($goods_listtmp){
                foreach ($goods_listtmp as $k=>$v){
					//0510 miao
						if ($_SESSION['mshenfen_id'] == 2 && $v['vipprice'] != '0.00'){
							$v['goods_promotion_price'] = $v['vipprice'];
						}elseif($_SESSION['mshenfen_id'] == 2 && $v['gjprice'] != '0.00'){
							$v['goods_promotion_price'] = $v['gjprice'];
						}
