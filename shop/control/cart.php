...
class cartControl extends BaseBuyControl {
    ...
    public function indexOp() {
        ...
        foreach ($cart_list as $cart) {
			...
			if($is_jibie == 1){
				if($cart['goods_vipprice'] != '0.00'){
					$cart['goods_price'] = $cart['goods_vipprice'];
				}
			}elseif($is_jibie == 2){
				if($cart['goods_gjprice'] != '0.00'){
					$cart['goods_price'] = $cart['goods_gjprice'];
				}
			}
    ...
    public function addOp() {
		...
		if($is_jibie == 1){
			if($goods_info['vipprice'] != '0.00'){
				$goods_info['goods_price'] = $goods_info['vipprice'];
			}
		}elseif($is_jibie == 2){
			if($goods_info['gjprice'] != '0.00'){
				$goods_info['goods_price'] = $goods_info['gjprice'];
			}
		}
        ...
        if ($insert) {
            //购物车商品种数记入cookie
            setNcCookie('cart_goods_num', $model_cart->cart_goods_num, 2 * 3600);//2天时间
    ...
    public function updateOp() {
		...
		if($is_jibie == 1){
			if($goods_info['goods_vipprice'] != '0.00'){
				$goods_info['goods_price'] = $goods_info['goods_vipprice'];
			}
		}elseif($is_jibie == 2){
			if($goods_info['goods_vipprice'] != '0.00'){
				$goods_info['goods_price'] = $goods_info['goods_vipprice'];
