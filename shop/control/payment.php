...
class paymentControl extends BaseHomeControl {
    ...
    private function _api_pay($order_info, $payment_info) {
        ...
        if ($payment_info['payment_code'] == 'allinpay') {
            $payment_api->submit();
        } elseif ($payment_info['payment_code'] == 'chinabank') {
            $payment_api->submit();
            ...
            @header("Location: " . $payment_api->get_payurl());//支付宝页面跳转
        }
        exit();
    }
    ...
    public function notifyOp() {
        ...
        //购买商品
        if ($order_type == 'real_order') {
            $result = $logic_payment->updateRealOrder($out_trade_no, $payment_info['payment_code'], $order_list, $trade_no, $order_datetime);
        } elseif ($order_type == 'vr_order') {
    ...
    public function returnOp() {
		...
		$order_datetime = $_GET['order_datetime'];
    ...
    public function query_stateOp() {
        ...
        } elseif (intval($_GET['pay_sn']) > 0) {
            $info = Model('predeposit')->getPdRechargeInfo(array('pdr_sn' => intval($_GET['pay_sn']), 'pdr_member_id' => intval($_GET['buyer_id'])));
