...
class store_voucherControl extends BaseSellerControl {
    ...
    public function templateaddOp() {
        ...
        if (chksubmit()) {
            ...
            if ($error) {
                ...
                $insert_arr['voucher_t_sc_id'] = intval($_POST['sc_id']); //商店类别id
                ...
                //新增3个字段
                $insert_arr['voucher_t_type'] = intval($_POST['voucher_t_type']);
                $insert_arr['voucher_t_type_id'] = $_POST['voucher_t_type_id'][$insert_arr['voucher_t_type']];
                if($insert_arr['voucher_t_type']!=0 && empty($insert_arr['voucher_t_type_id'])){
                    showMessage("缺少参数值,请重新填写", 'index.php?act=store_voucher&op=templatelist', 'html', 'error');
                }
               // print_r($insert_arr['voucher_t_type_id']);die;
                if (intval($_POST['voucher_t_type']) == 1) { //分类--》查询数据库
                    $rs = Model()->query("select gc_name from zmkj_goods_class where gc_id={$insert_arr['voucher_t_type_id']}");
                    $insert_arr['voucher_t_type_name'] = $rs[0]['gc_name'];
                } else {
                    $insert_arr['voucher_t_type_name'] = trim($_POST['voucher_t_type_name']);
                }
                //自定义图片
    ...
    public function templateeditOp() {
        ...
        $param['voucher_t_store_id'] = $_SESSION['store_id']; //代金券商店id
        $param['voucher_t_state'] = $this->templatestate_arr['usable'][0]; //代金券状态
        $param['voucher_t_giveout'] = array('elt', '0'); //代金券发出的数量
        $param['voucher_t_end_date'] = array('gt', time()); //代金券结束时间
        ...
        if (chksubmit()) {
            ...
            if ($error) {
                ...
                //新增3个字段
                $update_arr['voucher_t_type'] = intval($_POST['voucher_t_type']);
                $update_arr['voucher_t_type_id'] = $_POST['voucher_t_type_id'][$update_arr['voucher_t_type']];
                if($update_arr['voucher_t_type']!=0 && empty($update_arr['voucher_t_type_id'])){
                    showMessage("缺少参数值,请重新填写", 'index.php?act=store_voucher&op=templatelist', 'html', 'error');
                }
               // print_r($insert_arr['voucher_t_type_id']);die;
                if (intval($_POST['voucher_t_type']) == 1) { //分类--》查询数据库
                    $rs = Model()->query("select gc_name from zmkj_goods_class where gc_id={$update_arr['voucher_t_type_id']}");
                    $update_arr['voucher_t_type_name'] = $rs[0]['gc_name'];
                } else {
                    $update_arr['voucher_t_type_name'] = trim($_POST['voucher_t_type_name']);
                }
                // echo  $insert_arr['voucher_t_type_name'];die;
                //自定义图片
                if (!empty($_FILES['customimg']['name'])) {
                    $upload = new UploadFile();
                    $upload->set('default_dir', ATTACH_VOUCHER . DS . $_SESSION['store_id']);
            ...
            //获取分类的父级分类
            if($t_info['voucher_t_type']==1){
                $gc_id=$t_info['voucher_t_type_id'];
                //根据id获取父元素id
                $goods_class=Model("goods_class");
               $parent_class=$goods_class->getGoodsClassLineForTag($gc_id);
              $t_info['voucher_t_type_name']=$parent_class['gc_tag_name'];
            }
