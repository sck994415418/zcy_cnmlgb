...
class store_goods_addControl extends BaseSellerControl {
    ...
    public function save_goodsOp() {
        if (chksubmit()) {
            ...
            $common_array['goods_attr'] = serialize($attr); //二维数组[属性name_id][属性_value_id]，保存在goods_common表中
            //通过方法，将attr中手动输入框--取出来。          
            // 序列化保存手机端商品描述数据
            if ($_POST['m_body'] != '') {
            ...
            $common_array['vipprice'] = floatval($_POST['g_vip_price']);
            $common_array['gjprice'] = floatval($_POST['g_gj_price']);
            ...
            if ($common_id) {
                ...
                if (is_array($_POST['spec'])) {
                    foreach ($_POST['spec'] as $value) {
                        ...
                        $goods['vipprice'] = $value['vipprice'];
                        $goods['gjprice'] = $value['gjprice'];
                    ...
                    $goods['vipprice'] = $common_array['vipprice'];
                    $goods['gjprice'] = $common_array['gjprice'];
    ...
    public function add_step_threeOp() {
        ...
        if (!empty($img_array)) {
            ...
            foreach ($img_array as $val) {
                $image_array1[$val['color_id']][0]['goods_image'] = $val['goods_image'];
                $image_array1[$val['color_id']][0]['is_default'] = 1;
                $colorid_array1[] = $val['color_id'];
            }
            Tpl::output('img', $image_array1);
        }
    ...
    //商品参数列表
    public function goodsParamOp() {

        $goods_id = $_GET['goods_id']; //接受goods_id
        $edit_goods_sign = $_GET['edit_goods_sign'];
        Tpl::output('edit_goods_sign', $edit_goods_sign);
        //商品信息
        $goods_info = Model('goods')->query("select goods_param,gc_id from zmkj_goods where goods_id = " . $goods_id);
        //主体信息
        $title = Model()->query("select * from zmkj_title where class_id = " . $goods_info[0]['gc_id'] . " order by title_sort asc,title_id asc");
        if (empty($title)) {
            showMessage('没有关于此类商品的参数,请联系客服');
        }
        $goods_param = unserialize($goods_info[0]['goods_param']);
        //print_r($goods_param);
        //$array_key = array();
        foreach ($goods_param as $key1 => $value1) {
            foreach ($value1 as $key2 => $value2) {
                if (in_array($key2, $array_key)) {
                    unset($goods_param[$key1][$key2]);
                }
                $array_key[] = $key2;
            }
        }
        //如果保存了信息,将信息调出来
        Tpl::output('goods_param', $goods_param);
        Tpl::output('title', $title);
        Tpl::showpage("store_goods_param");
    }

    //参数保存
    public function spec_saveOp() {
        $goods_id = $_GET['goods_id'];
        $common_id = Model('goods')->query("select goods_commonid   from zmkj_goods where goods_id = $goods_id");
        $post = $_POST['at_value'];
        //过滤
        //先过滤内层
        ...
        if ($post == null) {
            ...
            $param = serialize($post);
            // var_dump($param);die;
            $goods = Model("goods");
            $update = $goods->editGoods(array('goods_param' => $param), array(goods_id => $goods_id));
            //$update = Model()->execute("update zmkj_goods set a goods_param = '".$attr."' where goods_id = $goods_id") 
            if ($update) {
                //判断:添加/编辑商品.
                if ($_GET['edit_goods_sign']) {
                    redirect(urlShop('store_goods_online', 'edit_image', array('commonid' => $common_id[0]['goods_commonid'])));
                } else {
                    redirect(urlShop('store_goods_add', 'add_step_three', array('commonid' => $common_id[0]['goods_commonid'])));
                }
            } else {
                showMessage('保存失败');
            }
    ...
    public function ajax_edit_goods_nameOp() {
        $goods_id = $_POST["goods_id"];
        //判断该商品是否是本店铺的商品.
        $store_id = $_SESSION['store_id'];
        $goods_info = Model()->query("select * from zmkj_goods where store_id={$store_id} and goods_id={$goods_id} and goods_state=1");
        if (empty($goods_info)) {
            exit(json_encode(array('state' => 0, 'msg' => '参数错误')));
        }
        $goods_name = $_POST["goods_name"];
        $rs = Model()->execute("update zmkj_goods set goods_name='" . $goods_name . "' where goods_id={$goods_id}");
        //print_r($goods_name);
        // print_r(Log::read());
        if ($rs) {
    ...
    public function add_indexerOp() {
        //echo 11;die;
        
        $indexer = Model('search');
        $rs=$indexer->one_update();
        //print_r($rs);
        //$rs = $indexer->one_update();
        //echo $rs;
    }
