...
class goodsControl extends BaseGoodsControl {
	...
	$_SESSION['goods_id'] = $goods_id;
        ...
        //推荐商品 v3-b12
        $goods_commend_list = $model_goods->getGoodsOnlineList(array('store_id' => $goods_info['store_id'], 'goods_commend' => 1), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_price,vipprice,gjprice', 0, 'rand()', 5, 'goods_commonid');
	...
	$good = 0;
		...
		$val['good'] > 0 ? $good += 1 : $good += 0;
		$val['normal'] > 0 ? $normal += 1 : $normal += 0;
		$val['bad'] > 0 ? $bad += 1 : $bad += 0;
		$val['all'] > 0 ? $all += 1 : $all += 0;
            //$good += $val['good'];
            //$normal += $val['normal'];
            //$bad += $val['bad'];
            //$all += $val['all'];
    ...
    public function commentsOp() {
        ...
        $this->_get_comments($str, $_GET['type'], 10);
     ...
     * 商品主题和参数
     */
    public function extendSpecOp() {
        $goods_id = intval($_GET['goods_id']);
        $goods = Model('goods');
        $goods_param = $goods->getGoodsInfo(array('goods_id'=>$goods_id),'goods_param');
        Tpl::output('goods_param', unserialize($goods_param['goods_param']));
        //print_r($goods_param['goods_param']);
        Tpl::showpage('goods.param', 'null_layout');
    }
    ...
    private function _get_comments($order_sn, $type, $page) {
        $brr = substr($order_sn, 1);
        $arr = explode(",", $brr);
        foreach ($arr as $v) {
            $condition = array();
            $condition['geval_orderno'] = $v;
            switch ($type) {
                case '1':
            ...
            //查询商品评分信息
		$condition['geval_goodsid'] = intval($_SESSION['goods_id']);
            $model_evaluate_goods = Model("evaluate_goods");
            $goodsevallist = $model_evaluate_goods->getEvaluateGoodsList($condition, $page);
            $goodscommon[] = $goodsevallist[0];
        }
        //var_dump($goodscommon);exit;
        foreach ($goodscommon as $key => $val) {
            if (empty($val)) {
                unset($goodscommon[$key]);
            }
        }
        //var_dump($goodscommon);exit;
        Tpl::output('goodscommon', $goodscommon);
        //print_r($goodscommon);
    ...
    public function salelogOp() {
        ...
        if ($_GET['vr']) {      //是否为虚拟商品
            ...
            $sales = $model_order->getOrderAndOrderGoodsSalesRecordList(array('goods_commonid' => $goods_commonid), '*', 10);
        } else {
            $model_order = Model('order');
            $sales = $model_order->getOrderAndOrderGoodsSalesRecordList(array('order_goods.goods_commonid' => $goods_commonid), 'order_goods.*, order.buyer_name, order.add_time', 10);
        }
        Tpl::output('show_page', $model_order->showpage());
        //var_dump($model_order->showpage());    //分页   		
        Tpl::output('sales', $sales);                 //order_goods表    goods_num   数量
        //var_dump($sales);
