...
//		Tpl::output('show_page',$page->show());
    ...
    public function album_pic_watermarkOp() {
        ...
        foreach ($wm_list as $v) {
            $filename = BASE_UPLOAD_PATH . DS . ATTACH_GOODS . DS . $_SESSION['store_id'] . DS . str_ireplace('.', '_1280.', $v['apic_cover']);
            //路径：原图片路径；利用原图片路径-->替换-->生成_1280图片
            if (file_exists($filename)) {
                unlink($filename);
                $coutents = file_get_contents(BASE_UPLOAD_PATH . DS . ATTACH_GOODS . DS . $_SESSION['store_id'] . DS . $v['apic_cover']);
                file_put_contents($filename, $coutents);
                $gd_image->create($filename);
            }    
        }
