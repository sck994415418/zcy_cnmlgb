...
class member_evaluateControl extends BaseMemberControl {
    ...
    /**
     * 订单添加评价
     * 表：订单表、信誉评价表、积分表、经验表、商品评价表、店铺评价表
     */
...
////////////////图片	
				if(!empty($_FILES)){
					$evaluate_imageArray = $_FILES['file']['name'][$value['goods_id']]['image'];					
					$evaluate_image_tmpArray=$_FILES['file']['tmp_name'][$value['goods_id']]['image'];
					$b = '/usr/share/nginx/html/data/upload/shop/common/';
					$q = 0;
					foreach($evaluate_imageArray as $k){
						$evaluate_image_url .= ','.UPLOAD_SITE_URL.'/shop/common/'.md5($k.date("h:i:s"));
						$a = $b.md5($k.date("h:i:s"));
						$w = move_uploaded_file($evaluate_image_tmpArray[$q],$a);
						$q++;						
					}
					$evaluate_image_url = substr($evaluate_image_url, 1);																													
					$evaluate_goods_info['geval_image'] = $evaluate_image_url;
				}
////////////////
...
//             //添加店铺评价
            ...
            //更新订单信息并记录订单日志
            $state = $model_order->editOrder(array('evaluation_state' => 3), array('order_id' => $order_id));
    ...
    public function second_addOp() {
        ...
        //订单为'已收货'状态，并且评论状态==3，且未被锁定。
        $order_info['evaluate_able'] = $model_order->getOrderOperateState('second_evaluation', $order_info);
		//var_dump($order_info['evaluate_able']);exit;
        if (empty($order_info) || !$order_info['evaluate_able']) {
            showMessage(Language::get('member_evaluation_order_notexists'), 'index.php?act=member_order', 'html', 'error');
        }
        //查询店铺信息
        ...
        if (!$_POST) {
            ...
            Tpl::output('order_goods', $order_goods);
            Tpl::output('store_info', $store_info);
            Tpl::showpage('evaluation_second.add');
        } else {
            //提交页面，更新操作:根据“订单id”和“商品id”确定评论的唯一性。
            $goods_id = $_POST['goods_id'];
            $evaluate_comments = $_POST['comment'];
            //更新操作。    
            $time = time();
//                $update = array();
//                $update['geval_second_content'] = $evaluate_comment; //时间戳
//                $update['geval_second_addtime'] = TIMESTAMP; //时间戳
//                $condition = array();
//                $condition['geval_orderid'] = $order_id; //订单id  
//                $condition['geval_goodsid'] = $value['goods_id']; //商品id
            $rs = Model()->execute("update zmkj_evaluate_goods set geval_second_content='{$evaluate_comments}',geval_second_addtime={$time} where geval_orderid={$order_id} and geval_goodsid={$goods_id}");
            if (!$rs) {
                showMessage(Language::get('member_second_evaluation_edit_fail'), 'index.php?act=member_order', 'html', 'error');
                return false;
            }
            $state = $model_order->editOrder(array('evaluation_state' => 1), array('order_id' => $order_id)); //状态为：追加评价后，为已评价
            //order_common:追加评论的时间。
            $model_order->editOrderCommon(array('evaluation_second_time' => TIMESTAMP), array('order_id' => $order_id));
            //order_log:订单日志表
            if ($state) {
                $data = array();
                $data['order_id'] = $order_id;
                $data['log_role'] = 'buyer';
                $data['log_msg'] = L('order_log_second_eval'); //快速调用语言包：进行追评
                $model_order->addOrderLog($data);
            }
            showDialog(Language::get('member_evaluation_evaluat_success'), 'index.php?act=member_order', 'succ');
        }
