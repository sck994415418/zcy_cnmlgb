...
class member_orderControl extends BaseMemberControl {
    ...
    public function indexOp() {
        ...
        //搜索--条件判断；
        $condition = array();
        $condition['is_master'] = 0;
        $condition['buyer_id'] = $_SESSION['member_id'];//买家id
        if ($_GET['order_sn'] != '') {
            $condition['order_sn'] = $_GET['order_sn'];//订单编号
        }
        $if_start_date = preg_match('/^20\d{2}-\d{2}-\d{2}$/', $_GET['query_start_date']);
        $if_end_date = preg_match('/^20\d{2}-\d{2}-\d{2}$/', $_GET['query_end_date']);
        $start_unixtime = $if_start_date ? strtotime($_GET['query_start_date']) : null;//开始时间
        $end_unixtime = $if_end_date ? strtotime($_GET['query_end_date']) : null;//结束时间
        if ($start_unixtime || $end_unixtime) { //如果存在，则执行。
            $condition['add_time'] = array('time', array($start_unixtime, $end_unixtime));
        }
        if ($_GET['state_type'] != '') {
            $condition['order_state'] = str_replace(
                    array('state_new', 'state_pay', 'state_send', 'state_success', 'state_noeval', 'state_cancel'), array(ORDER_STATE_NEW, ORDER_STATE_PAY, ORDER_STATE_SEND, ORDER_STATE_SUCCESS, ORDER_STATE_SUCCESS, ORDER_STATE_CANCEL), $_GET['state_type']);
        }
        ...
        $model_refund_return = Model('refund_return');//退款退货表
...
//        print_r($order_list);
        foreach ($order_list as $order_id => $order) {
            ...
            foreach ($order['extend_order_goods'] as $value) {
				if(!empty($value['goods_id'])){
					$a=Model()->query("select vipprice,gjprice from zmkj_goods where goods_id=".$value['goods_id']);
					if ($_SESSION['mshenfen_id'] == 2 && $a[0]['vipprice'] != '0.00'){
						$value['goods_price'] = $a[0]['vipprice'];
					}elseif($_SESSION['mshenfen_id'] == 3 && $a[0]['gjprice'] != '0.00'){
						$value['goods_price'] = $a[0]['gjprice'];
					} 
				}
        ...
        foreach ($order_group_list as $pay_sn => $pay_info) { //支付单号、支付信息
...
//        print_r($order_group_list);
    ...
    public function show_orderOp() {
        ...
        //显示评价
        $order_info['add'] = $model_order->getOrderOperateState('evaluation', $order_info);
