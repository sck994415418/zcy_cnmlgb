...
class predepositControl extends BaseMemberControl {
    ...
    public function recharge_addOp() {
        ...
        $pdr_amount = abs(floatval($_POST['pdr_amount'])); //绝对值
        if ($pdr_amount <= 0) {
            showMessage(Language::get('predeposit_recharge_add_pricemin_error'), '', 'html', 'error');
        }
        $model_pdr = Model('predeposit');
        $data = array();
        $data['pdr_sn'] = $pay_sn = $model_pdr->makeSn(); //充值唯一标志
        $data['pdr_member_id'] = $_SESSION['member_id']; //who
        $data['pdr_member_name'] = $_SESSION['member_name']; //
        $data['pdr_amount'] = $pdr_amount; //充值多少
        $data['pdr_add_time'] = TIMESTAMP; //时间
        $insert = $model_pdr->addPdRecharge($data);
        //默认的支付状态为0,所以不需要添加添加state
        if ($insert) {
            //转向到商城支付页面
    ...
    public function pd_cash_addOp() {
        if (chksubmit()) {
            ...
            $model_pd = Model('predeposit'); //预存款
            $model_member = Model('member'); //会员
            $member_info = $model_member->getMemberInfoByID($_SESSION['member_id']); //通过会员id,获取支付密码
            //验证支付密码
            ...
            //微信openid放入到pdc_bank_no中
            if ($_POST['pdc_bank_name'] == 0) {
                $member_id = $_SESSION['member_id'];
                $member = Model("member");
                $rs = $member->getMemberInfoByID($member_id);
                $weixin_info = unserialize($rs['weixin_info']);
                $_POST['pdc_bank_name'] = "微信";
                $_POST['pdc_bank_no'] = $weixin_info['openid'];
            }
            try {
                ...
                $data['pdc_payment_state'] = 0; //提现状态,默认为0
