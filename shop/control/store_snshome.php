		...
		if(!empty($strace_array) && is_array($strace_array)){
			foreach ($strace_array as $key=>$val){
				if($val['strace_content'] == ''){
					...
					//根据goodsid 查询商品vipprice miao
					if(!empty($val['strace_goodsdata']['goods_id'])){
						$a=Model()->query("select vipprice,gjprice from zmkj_goods where goods_id=".$val['strace_goodsdata']['goods_id']);
					
						if ($_SESSION['mshenfen_id'] == 2 && $a[0]['vipprice'] != '0.00'){
							$val['strace_goodsdata']['goods_price'] = $a[0]['vipprice'];
						}elseif($_SESSION['mshenfen_id'] == 3 && $a[0]['gjprice'] != '0.00'){
							$val['strace_goodsdata']['goods_price'] = $a[0]['gjprice'];
						} 
					}
					if( CHARSET == 'GBK') {
