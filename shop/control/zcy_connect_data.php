...
// 检测连接
		if (mysqli_connect_errno()) {
			$conn = @new mysqli($this->ip, $this->user, $this->pwd1, $this->dbname1, $this->port);
			if (mysqli_connect_errno()){
...
// 检测连接
		...
		if(!$rs){
			$rs_array = array("isSuccess"=>false,"resultMsg"=>mysqli_error($conn),"affectedRows"=>0);
		}else{
			$rs_array = array("isSuccess"=>true,"resultMsg"=>"成功","affectedRows"=>$affected_rows);
		}
		$conn->close();
		return $rs_array;
	}
