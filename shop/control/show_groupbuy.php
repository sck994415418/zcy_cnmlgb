...
class show_groupbuyControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        foreach ($groupbuy as $key => $value) {
            $goods_info = Model()->query("select vipprice,gjprice from zmkj_goods where goods_id={$value['goods_id']}");
            //print_r($goods_info);
            if ($_SESSION['mshenfen_id'] == 2 && $goods_info[0]['vipprice'] != 0) {
                $rs = $goods_info[0]['vipprice'] > $groupbuy[$key]['groupbuy_price'] ? $groupbuy[$key]['groupbuy_price'] : $goods_info[0]['vipprice'];
            } elseif ($_SESSION['mshenfen_id'] == 3 && $goods_info[0]['gjprice'] != 0) {
                $rs = $goods_info[0]['gjprice'] > $groupbuy[$key]['groupbuy_price'] ? $groupbuy[$key]['groupbuy_price'] : $goods_info[0]['gjprice'];
            } else {
                $rs = $groupbuy[$key]['groupbuy_price'];
            }
            $groupbuy[$key]['groupbuy_price'] = $rs;
            $groupbuy[$key]['groupbuy_rebate'] = round($rs / $groupbuy[$key]['goods_price'] * 10, 2);
        }
        //print_r($groupbuy);
    ...
    private function _show_groupbuy_list($function_name) {
        ...
        $groupbuy_list = $model_groupbuy->$function_name($condition, 20, $order);
        //获取vip价格/gj价格,将价格放入到groupbuy_price字段中.
        $goods_model = Model('goods');

        for ($i = 0; $i < count($groupbuy_list); $i++) {
            $aa[$i] = $goods_model->getGoodsList(array('goods_commonid' => $groupbuy_list[$i]['goods_commonid'])); //通过goods_commonid获取所有规格商品的库存
            $goods_info = Model()->query("select vipprice,gjprice from zmkj_goods where goods_id={$groupbuy_list[$i]['goods_id']}");
            if ($_SESSION['mshenfen_id'] == 2 && $goods_info[0]['vipprice'] != 0) {
                $rs = $goods_info[0]['vipprice'] > $groupbuy_list[$i]['groupbuy_price'] ? $groupbuy_list[$i]['groupbuy_price'] : $goods_info[0]['vipprice'];
            } elseif ($_SESSION['mshenfen_id'] == 3 && $goods_info[0]['gjprice'] != 0) {
                $rs = $goods_info[0]['gjprice'] > $groupbuy_list[$i]['groupbuy_price'] ? $groupbuy_list[$i]['groupbuy_price'] : $goods_info[0]['gjprice'];
            } else {
                $rs = $groupbuy_list[$i]['groupbuy_price'];
            }
            $groupbuy_list[$i]['groupbuy_price'] = $rs;
           $groupbuy_list[$i]['groupbuy_rebate'] = round($rs / $groupbuy_list[$i]['goods_price'] * 10, 2);
            for ($j = 0; $j < count($aa[$i]); $j++) {
                $groupbuy_list[$i]['goods_storage'] += $aa[$i][$j]['goods_storage']; //商品库存     
            }
        }
        Tpl::output('groupbuy_list', $groupbuy_list);
        Tpl::output('show_page', $model_groupbuy->showpage(5));
    ...
    public function groupbuy_detailOp() {
        ...
        //修改: 价格  折扣
        $goods_info = Model()->query("select vipprice,gjprice from zmkj_goods where goods_id={$groupbuy_info['goods_id']}");
        //print_r($goods_info);
        if ($_SESSION['mshenfen_id'] == 2 && $goods_info[0]['vipprice'] != 0) {
            $rs = $goods_info[0]['vipprice'] > $groupbuy_info['groupbuy_price'] ? $groupbuy_info['groupbuy_price'] : $goods_info[0]['vipprice'];
        } elseif ($_SESSION['mshenfen_id'] == 3 && $goods_info[0]['gjprice'] != 0) {
            $rs = $goods_info[0]['gjprice'] > $groupbuy_info['groupbuy_price'] ? $groupbuy_info['groupbuy_price'] : $goods_info[0]['gjprice'];
        } else {
            $rs = $groupbuy_info['groupbuy_price'];
        }
        $groupbuy_info['groupbuy_price'] = $rs;
        $groupbuy_info['groupbuy_rebate'] = round($rs / $groupbuy_info['goods_price'] * 10, 2);
