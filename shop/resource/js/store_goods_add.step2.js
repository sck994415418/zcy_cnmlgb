$(function(){
    ...
    $('dl[nctype="spec_group_dl"]').on('click', 'input[type="checkbox"]', function(){
        ...
        if(typeof(pv.find('input').val()) == 'undefined'){
            pv.html('<input type="text" maxlength="80" class="text" value="'+pv.html()+'" />');
        }else{
...
function computevipPrice(){
	// 计算最低价格
    var _price = 0;var _price_sign = false;
    $('input[data_type="vipprice"]').each(function(){
        if($(this).val() != '' && $(this)){
            if(!_price_sign){
                _price = parseFloat($(this).val());
                _price_sign = true;
    ...
    $('input[name="g_vip_price"]').val(number_format(_price, 2));

       // 计算折扣
}

// 计算高级价格
function computegjPrice(){
	
	// 计算最低价格
    var _price = 0;var _price_sign = false;
    $('input[data_type="gjprice"]').each(function(){
        if($(this).val() != '' && $(this)){
            if(!_price_sign){
                _price = parseFloat($(this).val());
                _price_sign = true;
    ...
    $('input[name="g_gj_price"]').val(number_format(_price, 2));
      // 计算折扣
}
