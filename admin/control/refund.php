...
class refundControl extends SystemControl{
	...
	public function refund_allOp() {
		...
		foreach ($refund_list as $key => $value) {
			...
			$order_info = Model()->table("order")->field("order_id,paytype,trade_no,paystate,payment_time")->where($map)->find();
			$refund_list[$key]['paytype'] = $order_info['paytype'];
			$refund_list[$key]['paystate'] = $order_info['paystate'];
			if($order_info['paytype'] == 'zfb'){
				$refund_list[$key]['pay']['WIDbatch_no']    = date("Ymd",time()).'00'.$order_info['order_id'];
	        	$refund_list[$key]['pay']['WIDbatch_num']   = 1;
	        	$refund_list[$key]['pay']['WIDdetail_data'] = $order_info['trade_no'].'^'.$value['refund_amount'].'^'.'订单退款';
        	}
        	if($order_info['paytype'] == 'wx'){
        		$refund_list[$key]['pay']['refund_fee']  = $value['refund_amount']*100;
	        	$refund_list[$key]['pay']['total_fee']   = $value['refund_amount']*100;
	        	$refund_list[$key]['pay']['transaction_id'] = $order_info['trade_no'];
        	}
			if($order_info['paytype'] == 'allinpay'){
        		$refund_list[$key]['pay']['refund_amount']  = $value['refund_amount']*100;
	        	$refund_list[$key]['pay']['order_no']   = $order_info['trade_no'];
	        	$refund_list[$key]['pay']['order_datetime'] = $order_info['payment_time'];
        	}
		}
		Tpl::output('refund_list',$refund_list);
		Tpl::output('show_page',$model_refund->showpage());
	...
	public function editOp() {
		...
		if (chksubmit()) {
			...
			// $state = Model()->table("refund_return")->where($refund)->update($refund);
			if ($state) {
