...
class predepositControl extends SystemControl {
    ...
    public function pd_cash_payOp() {
        ...
        $model_pd = Model('predeposit');
        $condition = array();
        $condition['pdc_id'] = $id; //id
        $condition['pdc_payment_state'] = 0; //支付状态
        $info = $model_pd->getPdCashInfo($condition);
     ...
     * 对接第三方转账
     */
    public function pd_cash_weixinOp($pdc_id=95) {
        //支付信息
        $result = Logic('payment')->getPaymentInfoAdmin('wxpay');
        if (!$result['state']) {
            Log::record('wxpay not found', 'RUN');
        }
        require_once BASE_ROOT_PATH . '/shop/api/payment/wxpay/lib/WxPay.Api.php';
        define('WXN_APPID', $result['data']['payment_config']['appid']);
        define('WXN_MCHID', $result['data']['payment_config']['mchid']);
        define('WXN_KEY', $result['data']['payment_config']['key']);
        //获取
        $predeposit = Model("predeposit");
        $condition['pdc_id']=$pdc_id;
        $rs = $predeposit->getPdCashInfo($condition);
        $openid = $rs['openid'];
        //统一下单
        $input = new WxPayUnifiedOrder();
        print_r($input);
        echo 11;
        $input->SetPartner_trade_no($rs['pdc_sn']); //商户订单号
           echo 33;
        $input->SetOpenid($openid);
        $input->SetCheck_name('FORCE_CHECK');
        $input->SetRe_user_name($rs['pdc_bank_user']);
        $input->SetAmount($rs['pdc_amount'] * 100);
        $input->SetDesc("提现");
        $input->SetSpbill_create_ip("39.104.66.197");
        echo 55;
        $result = WxPayApi::unifiedOrder1($input);
        print_r($result);die;
