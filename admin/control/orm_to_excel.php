...
if($is_bind_type == 1){
	$str = "sku\t商品名称\t政府商品id\t政府商品名称\t商城价格\n";
	...
	if(is_array($goods_orm_list)){
		foreach($goods_orm_list as $key=>$val){
			$sql1 = 'select goods_promotion_price from zmkj_goods where goods_id ='.$val['skuid'];
			$all_goods = Model()->query($sql1); //无重复的goods_id
			
			$str = $str. $val['skuid']."\t".$val['productNameEC']."\t".$val['productId']."\t".$val['productName']."\t".$all_goods[0]['goods_promotion_price']."\t0\n";
		}
	}
	$filename = time().'.xls';
