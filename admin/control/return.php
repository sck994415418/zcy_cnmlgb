...
class returnControl extends SystemControl{
	...
	public function return_allOp() {
		...
		foreach ($return_list as $key => $value) {
			...
			$order_info = Model()->table("order")->field("order_id,paytype,trade_no,paystate")->where($map)->find();
			$return_list[$key]['paytype'] = $order_info['paytype'];
			$return_list[$key]['paystate'] = $order_info['paystate'];
			if($order_info['paytype'] == 'zfb'){
				$return_list[$key]['pay']['WIDbatch_no']    = date("Ymd",time()).'00'.$order_info['order_id'];
	        	$return_list[$key]['pay']['WIDbatch_num']   = 1;
	        	$return_list[$key]['pay']['WIDdetail_data'] = $order_info['trade_no'].'^'.$value['refund_amount'].'^'.'订单退货退款';
        	}
        	if($order_info['paytype'] == 'wx'){
        		$return_list[$key]['pay']['refund_fee']  = $value['refund_amount']*100;
	        	$return_list[$key]['pay']['total_fee']   = $value['refund_amount']*100;
	        	$return_list[$key]['pay']['transaction_id'] = $order_info['trade_no'];
        	}
		}
		Tpl::output('return_list',$return_list);
		Tpl::output('show_page',$model_refund->showpage());
