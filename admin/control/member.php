...
class memberControl extends SystemControl{
	...
	public function memberOp(){
		...
		//0507 MIAO 会员身份等级
		$mshenfen_id = $_GET['mshenfen_id'];
		if(!empty($mshenfen_id) && $mshenfen_id > 0){
			$condition['mshenfen_id'] = $mshenfen_id;
		}
		elseif($mshenfen_id == -2){
			$condition['mshenfen_id'] = 0;	
		}
		//0507 MIAO 会员级别身份
		$mshenfen = Model('mshenfen');
		$mshenfen_list = $mshenfen->field('mshenfen_id,mshenfen_name')->where('is_show = 1')->order('mshenfen_sort desc')->select();
		Tpl::output('mshenfen_list',$mshenfen_list);
		...
		if (is_array($member_list)){
			foreach ($member_list as $k=> $v){
				...
				$mshenfen = Model('mshenfen');
				$mshenfen_name = $mshenfen->field('mshenfen_name')->where('mshenfen_id = '.$v['mshenfen_id'])->order('mshenfen_sort desc')->find();
				$member_list[$k]['mshenfen_name'] = $mshenfen_name['mshenfen_name'];
			}
		}
	...
	public function member_editOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				$update_array['is_master'] 		= $_POST['is_master'];
				//v3-b11 新增
				...
				$update_array['mshenfen_id'] 	= intval($_POST['mshenfen_id']);
	...
	public function member_addOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				$insert_array['mshenfen_id'] 	= intval($_POST['mshenfen_id']);
                //默认允许举报商品
