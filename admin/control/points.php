...
class pointsControl extends SystemControl{
	...
	public function ruleOp(){
		...
		$set[] = 'points_reg';
		$set[] = 'points_login';
		$set[] = 'points_orderrate';
		$set[] = 'points_comments';
		$set[] = 'points_invite';
		$set[] = 'points_rebate';
		//循环查询到几个值并输出到页面
		foreach($set as $key => $v){
			$sql = "select `value` from zmkj_setting where `name` = '".$v."'";
			$value[] = $model_setting->query($sql);
		}
		Tpl::output('value',$value);
		Tpl::showpage('rule');
	}
	/*
	 * 保存积分规则
	...
	public function editOp(){
		...
		if (chksubmit()){
			...
			foreach($post as $key => $v){
				$sql = "update zmkj_setting set `value` = '".$v."' where `name` = '".$key."'";
				$query = $model_setting->execute($sql);
			}
			//如果修改成功,跳转到上一页,修改失败也跳转到上一页
			if($query){
				echo "<script>alert('修改成功!');location.href='".$_SERVER["HTTP_REFERER"]."';</script>";
			}else{
				echo "<script>alert('修改失败,请重试!');location.href='".$_SERVER["HTTP_REFERER"]."';</script>";
			}
		}
	}
	/**
