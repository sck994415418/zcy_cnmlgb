...
class orderControl extends SystemControl{
	...
	public function indexsOp(){
		...
		$memap['is_master'] = 1;
		$member = Model()->table("member")->field("member_id")->where($memap)->select();
		$arr = array();
		foreach ($member as $key => $value) {
			$arr[] = $value['member_id']; 
		}
		$str = implode(',', $arr);
		$condition['pdc_member_id'] = array('in',$str);
	...
	public function show_orderOp(){
		...
		foreach ($order_info['extend_order_goods'] as $key=>$value) {
			if(!empty($value['goods_id'])){
					$member_id=Model()->query("select mshenfen_id from zmkj_member where member_id=".$order_info['buyer_id']);
					$a=Model()->query("select vipprice,gjprice from zmkj_goods where goods_id=".$value['goods_id']);
					
					if ($member_id[0]['mshenfen_id'] == 2 && $a[0]['vipprice'] != '0.00'){
						$value['goods_price'] = $a[0]['vipprice'];
						$value['goods_pay_price'] = $a[0]['vipprice']*$value['goods_num'];
					}elseif($member_id[0]['mshenfen_id'] == 3 && $a[0]['gjprice'] != '0.00'){
						$value['goods_price'] = $a[0]['gjprice'];
						$value['goods_pay_price'] = $a[0]['gjprice']*$value['goods_num'];
					} 
					$order_info['extend_order_goods'][$key] = $value;
			}
		}
		Tpl::output('order_info',$order_info);
