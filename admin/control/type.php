...
class typeControl extends SystemControl {
    ...
    public function typeOp() {
        ...
        $page->setEachNum(10);
        $page->setStyle('admin');
        $type_list = $model_type->typeList(array('order' => 'type_sort asc,type_id asc'), $page);
            ...
            if (!empty($_POST['at_value'])) {
                ...
                foreach ($attribute_array as $v) {
                    if ($v['value'] != '') {
                        ...
                        //添加属性
                        $attr_array = array();
                        $attr_array['attr_name'] = $v['name']; //属性名
                        $attr_array['attr_value'] = $v['value']; //属性值
                        $attr_array['type_id'] = $type_id; //主题id
                        $attr_array['attr_sort'] = $v['sort']; //属性排序
                        $attr_array['attr_show'] = $v['show']; //属性：是否显示

                        $attr_id = $model_type->typeAdd('attribute', $attr_array); //attribute表                
                        if (!$attr_id) {
                            showMessage($lang['type_index_related_fail']);
                        }

                        //添加属性值
                        $attr_value = explode(',', $v['value']); //数组
                        if (!empty($attr_value)) {//如果数组不为空,则执行这个.
                            $attr_array = array();
                            ...
                            $return = Model('attribute')->addAttributeValueAll($attr_array); //attribute_value表     
                            if (!$return) {
                                showMessage($lang['type_index_related_fail']);
                            }
                        }
...
//			case 'name':
                $return = $model_type->typeUpdate(
                        array($_GET['column'] => trim($_GET['value'])), array('type_id' => intval($_GET['id'])), 'type'
                );
                if ($return) {
