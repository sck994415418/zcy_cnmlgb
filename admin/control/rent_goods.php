...
class rent_goodsControl extends SystemControl {
    ...
    public function goods_setOp() {
        ...
        if (chksubmit()) {
            ...
            if ($result === true) {
                $this->log(L('nc_edit,nc_goods_set'), 1); //记入日志
...
// 全部商品
            default:
                $goods_list = $model_goods->getGoodsCommonList($where); //goods_common表
                break;
        }
//调出所有的商品.
...
//验证
        $rent_goods_info = Model()->query("select * from zmkj_goods_rent where goods_id={$goods_id}");
        if (empty($rent_goods_info)) {
            exit(json_encode(array('state' => false, 'msg' => '该商品不存在')));
        }
        $goods_rent_type = Model()->query("select * from zmkj_goods_rent_class where goods_id={$goods_id} order by rent_id asc");
        if (empty($goods_rent_type)) {
            exit(json_encode(array('state' => false, 'msg' => '没有添加租赁方式')));
        }
//输出：租金、租期、租赁方式
        $goods_rent_type = "";
        foreach ($goods_rent_type as $key => $value) {
            $goods_rent_type['rent_type'] .= "<div class='goods_spec'>租金:<em title=" . $value['zujin'] . ">" . $value['zujin'] . "</em></div>";
            $goods_rent_type['rent_type'] .= "<div class = 'goods_spec'>租期:<em title = " . $value['name'] . ">" . $value['name'] . "</em></div>";
            $goods_rent_type['rent_type'] .= "<div class = 'goods_spec'>租赁方式:<em title = " . $value['fangshi'] . ">" . $value['fangshi'] . "</em></div>";
        }
        exit(json_encode(array('state' => true, 'msg' => '成功获取数据', 'data' => $goods_rent_type)));//三维数组
    }

//<div class = "goods_spec">品牌：<em title = "办工师">办工师</em></div><div class = "goods_spec">颜色：<em title = "白色">白色</em></div><div class = "goods_spec">型号：<em title = "57*50mm 200卷/箱">57*50mm 200卷/箱</em></div>"
}
