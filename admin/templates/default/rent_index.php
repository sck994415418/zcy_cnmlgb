                ...
                <?php if (!empty($output['goods_list']) && is_array($output['goods_list'])) { ?>
                    <?php foreach ($output['goods_list'] as $k => $v) { ?>
                        <tr class="hover edit">
                            ...
                            <td><i class="icon-plus-sign" style="cursor: pointer;" nctype="ajaxGoodsList" data-goodsid="<?php echo $v['goods_id']; ?>" title="点击展开查看此商品全部规格；规格值过多时请横向拖动区域内的滚动条进行浏览。"></i></td>
                            <td class="align-center"><?php echo $v['rent_id']; ?></td>
                        ...
                        $(function () {
                            ...
                            $('i[nctype="ajaxGoodsList"]').toggle(
                                    function () { //显示
                                        ...
                                        var _parenttr = $(this).parents('tr');//jquery对象
                                        var _goods_id = $(this).attr('data-goodsid');//值
                                        var _div = _parenttr.next().find('.ncsc-goods-sku');//jQuery对象
                                        if (_div.html() == '') { //空，进行ajax
                                            $.getJSON('index.php?act=rent_goods&op=get_goods_list_ajax', {goods_id: _goods_id}, function (date) {
                                                if (date['state']!= 'false') {
                                                    var _ul = $('<ul class="ncsc-goods-sku-list"></ul>');
                                                    $.each(date['data'], function (i, o) {
                                                        $('<li>' + o.rent_type + '</li>').appendTo(_ul);
                                                    });
                                                    _ul.appendTo(_div);
                                                    _parenttr.next().show();
                                                    // 计算div的宽度
