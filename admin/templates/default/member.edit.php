  ...
  <form id="user_form" enctype="multipart/form-data" method="post">
    ...
    <table class="table tb-type2">
      <tbody>
		...
		<!-- 2018-5-3添加--> 
 		<tr>
          <td colspan="2" class="required"><label class="member_areainfo">会员级别身份:</label></td>
        </tr>
		<tr class="noborder">
			<td class="vatop rowform"  colspan="2">
				<span class="w400">
					<select id="mshenfen_id" name="mshenfen_id">
						<?php if ($output['mshenfen_list']){?>
						  <?php foreach ($output['mshenfen_list'] as $k=>$v){?>
								<option <?php if(isset($output['mshenfen_list']) && $output['member_array']['mshenfen_id'] == $v['mshenfen_id']){ ?>selected='selected'<?php } ?> value="<?php echo $v['mshenfen_id'];?>"><?php echo $v['mshenfen_name'];?></option>
						  <?php }?>
						  <?php }?>
					</select>
				</span>
        ...
        <tr class="noborder">
         ...
         <tr>
          <td colspan="2" class="required"><label class="member_qq">师傅:</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform">
            <input type="checkbox" <?php if($output['member_array']['is_master']){ ?>checked<?php } ?> name="is_master" value="1">是
          </td>
          <td class="vatop tips">
          </td>
        </tr>
