  ...
  <table class="table tb-type2 nobdb">
    ...
    <tbody>
      ...
      <tr class="hover">
        ...
        <td class="align-center"><?php echo $order['cost'];?></td>
        <td class="align-center"><?php echo $order['state_desc'];?></td>
        <td class="align-center"><?php echo $order['master'];?></td>
        <td class="w144 align-center">
          <a href="index.php?act=vr_order&op=show_order&order_id=<?php echo $order['order_id'];?>">
            <?php echo $lang['nc_view'];?>
          </a>

            <!-- 取消订单 -->
  		    <?php if($order['wxstate'] <= 1) {?>
        	| <a href="javascript:void(0)" onclick="if(confirm('<?php echo $lang['order_confirm_cancel'];?>')){location.href='index.php?act=vr_order&op=change_state&state_type=cancel&order_id=<?php echo $order['order_id']; ?>'}">
        	  <?php echo $lang['order_change_cancel'];?>
          </a>
        	<?php }?>

          <?php if($order['wxstate'] == 5 && $order['paytype'] == 'zfb' && $order['paystate'] == 0) {?>
          | <a href="/shop/refund_fastpay/alipayapi.php?WIDbatch_no=<?php echo $order['pay']['WIDbatch_no']; ?>&WIDbatch_num=<?php echo $order['pay']['WIDbatch_num']; ?>&WIDdetail_data=<?php echo $order['pay']['WIDdetail_data']; ?>">
            退款
          </a>
          <?php }?>

          <?php if($order['wxstate'] == 5 && $order['paytype'] == 'wx' && $order['paystate'] == 0) {?>
          | <a href="/shop/wxpayapi/example/refund.php?transaction_id=<?php echo $order['pay']['transaction_id']; ?>&total_fee=<?php echo $order['pay']['total_fee']; ?>&refund_fee=<?php echo $order['pay']['refund_fee']; ?>">
            退款
          </a>
          <?php }?>

          <?php if($order['paystate'] == 1) {?>
          |已退款
          <?php }?>
        	<!-- 收款 
