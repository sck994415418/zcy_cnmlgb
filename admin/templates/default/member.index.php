      ...
      <tbody>
        <tr>
          ...
          <td>
		   ...
		   <td>
            <select name="mshenfen_id" >
              <option value='-1'>会员级别身份</option>
			  <option <?php if(isset($_GET['mshenfen_id']) && $_GET['mshenfen_id'] == 0){ ?>selected='selected'<?php } ?> value='-2'>未设置身份</option>
			   <?php if ($output['mshenfen_list']){?>
              <?php foreach ($output['mshenfen_list'] as $k=>$v){?>
				<option <?php if(isset($_GET['mshenfen_id']) && $_GET['mshenfen_id'] == $v['mshenfen_id']){ ?>selected='selected'<?php } ?> value="<?php echo $v['mshenfen_id'];?>"><?php echo $v['mshenfen_name'];?></option>
              <?php }?>
              <?php }?>
            </select>
          </td>
        ...
        <thead>
          <tr class="thead">
            ...
            <th colspan="2"><?php echo $lang['member_index_name']?></th>
			 <th class="align-center">会员级别身份</th>
            <th class="align-center">会员身份</th>
          ...
          <tbody>
            ...
            <tr class="hover member">
              <td class="w24">
                <input type="checkbox" name='del_id[]' value="<?php echo $v['member_id']; ?>" class="checkitem">
              </td>
              <td class="w48 picture">
                <div class="size-44x44">
                  <span class="thumb size-44x44"><i></i><img src="<?php if ($v['member_avatar'] != ''){ echo UPLOAD_SITE_URL.DS.ATTACH_AVATAR.DS.$v['member_avatar'];}else { echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('default_user_portrait');}?>?<?php echo microtime();?>"  onload="javascript:DrawImage(this,44,44);"/>
                  </span>
                </div>
              </td>
              <td>
                <p class="name">
                  ...
                  (<?php echo $lang['member_index_true_name']?>: <?php echo $v['member_truename']; ?>)
                </p>
                <p class="smallfont">
                  <?php echo $lang['member_index_reg_time']?>:&nbsp;<?php echo $v['member_time']; ?>
                </p>
			   ...
			   <!-- 2018-5-3 新添加的功能-->
			  <td class="w48 picture">
				<?php if($v['mshenfen_id'] == 0){ ?>
					未设置身份
                <?php }else{ ?>
					<?php echo $v['mshenfen_name'];?>
				<?php } ?>
              </td>
              <td class="w48 picture">
                <?php if($v['is_master'] == 1){ ?>
                  维修师傅
                <?php }else{ ?>
                  普通会员
                <?php } ?>
              ...
              <td class="w150 align-center"><p><?php echo $v['member_login_time']; ?></p>
                <p><?php echo $v['member_login_ip']; ?></p>
              </td>
              ...
              <td class="align-center"><?php echo $v['member_state'] == 1?$lang['member_edit_allow']:$lang['member_edit_deny']; ?>
              </td>
              <td class="align-center"><a href="index.php?act=member&op=member_edit&member_id=<?php echo $v['member_id']; ?>"><?php echo $lang['nc_edit']?></a> | <a href="index.php?act=notice&op=notice&member_name=<?php echo ltrim(base64_encode($v['member_name']),'='); ?>"><?php echo $lang['member_index_to_message'];?></a>
              </td>
            </tr>
            <?php } ?>
            <?php }else { ?>
